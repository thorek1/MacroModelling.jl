...
# Keyword: Core.kwcall(kwargs, get_loglikelihood, model, data, params)
@is_primitive Mooncake.DefaultCtx Tuple{typeof(Core.kwcall), <:NamedTuple, typeof(MacroModelling.get_loglikelihood), MacroModelling.ℳ, KeyedArray{Float64}, Vector{T}} where {T<:Base.IEEEFloat}
...
# above do not match during Mooncake's abstract interpretation, causing it
# to trace into the full function body (~700s). These wider declarations
# ensure the primitive is recognized. The concrete-typed rrule!! methods
# above still handle runtime dispatch correctly.
