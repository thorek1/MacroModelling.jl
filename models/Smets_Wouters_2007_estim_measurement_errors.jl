@model Smets_Wouters_2007 begin
	...
	dy[0] = ctrend + 100 * (y[0] / y[-1] - 1) + z_dy * e_dy[x]

	dc[0] = ctrend + 100 * (c[0] / c[-1] - 1) + z_dc * e_dc[x]

	dinve[0] = ctrend + 100 * (inve[0] / inve[-1] - 1) + z_dinve * e_dinve[x]

	pinfobs[0] = 100 * (pinf[0] - 1) + z_pinfobs * e_pinfobs[x]

	robs[0] = 100 * (r[0] - 1) + z_robs * e_robs[x]
