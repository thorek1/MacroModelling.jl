@model Smets_Wouters_2007 begin
	...
	r[0] = r[ss] ^ (1 - crr) * r[-1] ^ crr * (pinf[0] / cpie * exp(σ_pinf[0])) ^ ((1 - crr) * crpi) * (pinf[4] / cpie * exp(σ_pinf_4[0])) ^ ((1 - crr) * crpi_4) * (y[0] / yflex[0]) ^ ((1 - crr) * cry) * (y[0] / yflex[0] / (y[-1] / yflex[-1])) ^ crdy * ms[0]
    ...
    σ_pinf_4[0] = ρ_pinf_4 * σ_pinf_4[-1] + z_pinf_4 * eσ_pinf_4[x]
