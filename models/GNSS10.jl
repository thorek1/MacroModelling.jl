...
@model GNSS10 begin
	...
	exp(R_b[0]) = exp(r_ib[0]) + ( - kappa_kb) * (exp(K_b[0]) / exp(B[0]) - vi) * (exp(K_b[0]) / exp(B[0])) ^ 2
	...
	kappa_d * beta_p * exp(lam_p[1]) / exp(lam_p[0]) * (exp(r_d[1]) / exp(r_d[0]) - (exp(r_d[0]) / exp(r_d[-1])) ^ ind_d) * (exp(r_d[1]) / exp(r_d[0])) ^ 2 * exp(d_b[1]) / exp(d_b[0]) + exp(mk_d[0]) / (exp(mk_d[0]) - 1) - 1 - exp(r_ib[0]) * exp(mk_d[0]) / (exp(mk_d[0]) - 1) / exp(r_d[0]) - exp(r_d[0]) * kappa_d * (exp(r_d[0]) / exp(r_d[-1]) - (exp(r_d[-1]) / exp(r_d[-2])) ^ ind_d) / exp(r_d[-1]) = 0
	...
	1 + exp(r_ib[0]) = (1 + r_ib_ss) ^ (1 - rho_ib) * (1 + exp(r_ib[-1])) ^ rho_ib * ((exp(pie[0]) / piss) ^ phi_pie * (exp(Y1[0]) / exp(Y1[-1])) ^ phi_y) ^ (1 - rho_ib) * (1 + e_r_ib[x])
