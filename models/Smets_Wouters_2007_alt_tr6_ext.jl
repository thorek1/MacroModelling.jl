@model Smets_Wouters_2007 begin
	...
	r[0] = r[ss] ^ (1 - crr) * r[-1] ^ crr * (pinf[0] / cpie * exp(σ_pinf[0])) ^ ((1 - crr) * crpi) * (pinf[6] / cpie * exp(σ_pinf_6[0])) ^ ((1 - crr) * crpi_6) * (y[0] / yflex[0]) ^ ((1 - crr) * cry) * (y[0] / yflex[0] / (y[-1] / yflex[-1])) ^ crdy * ms[0]
    ...
    σ_pinf[0] = ρ_pinf * σ_pinf[-1] + z_pinf * eσ_pinf[x]

    σ_pinf_6[0] = ρ_pinf_6 * σ_pinf_6[-1] + z_pinf_6 * eσ_pinf_6[x]
	...
	dlabobs[0] = constelab + 100 * (lab[0] / lab[-1] - 1) # + z_dlabobs / 100 * e_dlabobs[x]
