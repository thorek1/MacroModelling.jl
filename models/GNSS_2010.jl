...
@model GNSS_2010 begin
	...
	interestH[0] = 400 * r_bh[0]

	interestF[0] = 400 * r_be[0]
	...
	exp(lam_p[0]) = beta_p * exp(lam_p[1]) * (1 + r_d[0]) / exp(pie[1])
	...
	exp(c_p[0]) + exp(q_h[0]) * (exp(h_p[0]) - exp(h_p[-1])) + exp(d_p[0]) = exp(l_p[0]) * exp(w_p[0]) + (1 + r_d[-1]) * exp(d_p[-1]) / exp(pie[0]) + exp(J_R[0]) / gamma_p
	...
	exp(lam_i[0]) - beta_i * exp(lam_i[1]) * (1 + r_bh[0]) / exp(pie[1]) = exp(s_i[0]) * (1 + r_bh[0])
	...
	exp(c_i[0]) + exp(q_h[0]) * (exp(h_i[0]) - exp(h_i[-1])) + (1 + r_bh[-1]) * exp(b_i[-1]) / exp(pie[0]) = exp(l_i[0]) * exp(w_i[0]) + exp(b_i[0])

	(1 + r_bh[0]) * exp(b_i[0]) = exp(pie[1]) * exp(h_i[0]) * exp(q_h[1]) * exp(m_i[0])
	...
	(1 - deltak) * exp(pie[1]) * exp(q_k[1]) * exp(s_e[0]) * exp(m_e[0]) + beta_e * exp(lam_e[1]) * ((1 - deltak) * exp(q_k[1]) + r_k[1] * exp(u[1]) - (eksi_1 * (exp(u[1]) - 1) + eksi_2 / 2 * (exp(u[1]) - 1) ^ 2)) = exp(q_k[0]) * exp(lam_e[0])

	exp(w_p[0]) = ni * (1 - alpha) * exp(y_e[0]) / (exp(l_pd[0]) * exp(x[0]))

	exp(w_i[0]) = exp(y_e[0]) * (1 - alpha) * (1 - ni) / (exp(x[0]) * exp(l_id[0]))

	exp(lam_e[0]) - exp(s_e[0]) * (1 + r_be[0]) = beta_e * exp(lam_e[1]) * (1 + r_be[0]) / exp(pie[1])

	r_k[0] = eksi_1 + eksi_2 * (exp(u[0]) - 1)

	exp(c_e[0]) + (1 + r_be[-1]) * exp(b_ee[-1]) / exp(pie[0]) + exp(w_p[0]) * exp(l_pd[0]) + exp(w_i[0]) * exp(l_id[0]) + exp(q_k[0]) * exp(k_e[0]) + (eksi_1 * (exp(u[0]) - 1) + eksi_2 / 2 * (exp(u[0]) - 1) ^ 2) * exp(k_e[-1]) = exp(y_e[0]) / exp(x[0]) + exp(b_ee[0]) + exp(k_e[-1]) * (1 - deltak) * exp(q_k[0])

	exp(y_e[0]) = exp(A_e[0]) * (exp(u[0]) * exp(k_e[-1])) ^ alpha * (exp(l_pd[0]) ^ ni * exp(l_id[0]) ^ (1 - ni)) ^ (1 - alpha)

	(1 + r_be[0]) * exp(b_ee[0]) = (1 - deltak) * exp(k_e[0]) * exp(pie[1]) * exp(q_k[1]) * exp(m_e[0])

	r_k[0] = (exp(l_pd[0]) ^ ni * exp(l_id[0]) ^ (1 - ni)) ^ (1 - alpha) * alpha * exp(A_e[0]) * exp(u[0]) ^ (alpha - 1) * exp(k_e[-1]) ^ (alpha - 1) / exp(x[0])

	R_b[0] = r_ib[0] + ( - kappa_kb) * (exp(K_b[0]) / exp(B[0]) - vi) * (exp(K_b[0]) / exp(B[0])) ^ 2
	...
	kappa_d * beta_p * exp(lam_p[1]) / exp(lam_p[0]) * (r_d[1] / r_d[0] - (r_d[0] / r_d[-1]) ^ ind_d) * (r_d[1] / r_d[0]) ^ 2 * exp(d_b[1]) / exp(d_b[0]) + exp(mk_d[0]) / (exp(mk_d[0]) - 1) - 1 - r_ib[0] * exp(mk_d[0]) / (exp(mk_d[0]) - 1) / r_d[0] - r_d[0] * kappa_d * (r_d[0] / r_d[-1] - (r_d[-1] / r_d[-2]) ^ ind_d) / r_d[-1] = 0

	beta_p * exp(lam_p[1]) / exp(lam_p[0]) * kappa_be * (r_be[1] / r_be[0] - (r_be[0] / r_be[-1]) ^ ind_be) * (r_be[1] / r_be[0]) ^ 2 * exp(b_e[1]) / exp(b_e[0]) + 1 - exp(mk_be[0]) / (exp(mk_be[0]) - 1) + R_b[0] * exp(mk_be[0]) / (exp(mk_be[0]) - 1) / r_be[0] - r_be[0] * kappa_be * (r_be[0] / r_be[-1] - (r_be[-1] / r_be[-2]) ^ ind_be) / r_be[-1] = 0

	beta_p * exp(lam_p[1]) / exp(lam_p[0]) * kappa_bh * (r_bh[1] / r_bh[0] - (r_bh[0] / r_bh[-1]) ^ ind_bh) * (r_bh[1] / r_bh[0]) ^ 2 * exp(b_h[1]) / exp(b_h[0]) + 1 - exp(mk_bh[0]) / (exp(mk_bh[0]) - 1) + R_b[0] * exp(mk_bh[0]) / (exp(mk_bh[0]) - 1) / r_bh[0] - r_bh[0] * kappa_bh * (r_bh[0] / r_bh[-1] - (r_bh[-1] / r_bh[-2]) ^ ind_bh) / r_bh[-1] = 0

	exp(j_B[0]) = r_bh[0] * exp(b_h[0]) + r_be[0] * exp(b_e[0]) - r_d[0] * exp(d_b[0]) - exp(d_b[0]) * r_d[0] * kappa_d / 2 * (r_d[0] / r_d[-1] - 1) ^ 2 - exp(b_e[0]) * r_be[0] * kappa_be / 2 * (r_be[0] / r_be[-1] - 1) ^ 2 - exp(b_h[0]) * r_bh[0] * kappa_bh / 2 * (r_bh[0] / r_bh[-1] - 1) ^ 2 - exp(K_b[0]) * kappa_kb / 2 * (exp(K_b[0]) / exp(B[0]) - vi) ^ 2
	...
	rr_e[0] = exp(lam_e[0]) - beta_e * exp(lam_e[1]) * (1 + r_be[0]) / exp(pie[1])

	aux1[0] = exp(K_b[0]) / exp(B[0])

	exp(bm[0]) = r_bh[-1] * exp(b_h[-1]) / (exp(b_h[-1]) + exp(b_e[-1])) + r_be[-1] * exp(b_e[-1]) / (exp(b_h[-1]) + exp(b_e[-1])) - r_d[-1]

	exp(spr_b[0]) = r_bh[0] * 0.5 + r_be[0] * 0.5 - r_d[0]
end
