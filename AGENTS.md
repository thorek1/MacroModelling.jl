...
3. **Use Revise-based development:** Keep one Julia REPL running (`julia -t auto --project=.`), load `Revise` before `MacroModelling`, and iterate in that session. On Linux machines, Julia installed via juliaup can be found in `~/.juliaup/bin`. Install missing packages when they are not present in the active environment.
