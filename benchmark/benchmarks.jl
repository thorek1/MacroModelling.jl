...
function run_benchmarks!(𝓂::ℳ, SUITE::BenchmarkGroup)
    ...
    sol, qme_sol, solved = calculate_first_order_solution(∇₁, 𝓂.cache; opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :schur))
    
    clear_solution_caches!(𝓂, :first_order)
    
    SUITE[𝓂.model_name]["qme"]["schur"] = @benchmarkable calculate_first_order_solution($∇₁, $𝓂.cache; opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :schur)) setup = clear_solution_caches!($𝓂, :first_order)
    
    SUITE[𝓂.model_name]["qme"]["doubling"] = @benchmarkable calculate_first_order_solution($∇₁, $𝓂.cache; opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :doubling)) setup = clear_solution_caches!($𝓂, :first_order)
