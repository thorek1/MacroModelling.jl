...
SUITE["FS2000"]["qme"]["schur"] = @benchmarkable calculate_first_order_solution($∇₁; T = $𝓂.timings, opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :schur)) setup = clear_solution_caches!($𝓂, :first_order)
...
SUITE["NAWM_EAUS_2008"]["qme"]["schur"] =       @benchmarkable calculate_first_order_solution($∇₁; T = $𝓂.timings, opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :schur)) setup = clear_solution_caches!($𝓂, :first_order)
...
𝓂 = Smets_and_Wouters_2007

SUITE["Smets_and_Wouters_2007"] = BenchmarkGroup()
...
SUITE["Smets_and_Wouters_2007"]["irf"] = @benchmarkable get_irf($𝓂) setup = clear_solution_caches!($𝓂, :first_order)

reference_steady_state, (solution_error, iters) = get_NSSS_and_parameters(𝓂, 𝓂.parameter_values)

clear_solution_caches!(𝓂, :first_order)

SUITE["Smets_and_Wouters_2007"]["NSSS"] = @benchmarkable get_NSSS_and_parameters($𝓂, $𝓂.parameter_values) setup = clear_solution_caches!($𝓂, :first_order)
...
SUITE["Smets_and_Wouters_2007"]["jacobian"] = @benchmarkable calculate_jacobian($𝓂.parameter_values, $reference_steady_state, $𝓂) setup = clear_solution_caches!($𝓂, :first_order)


SUITE["Smets_and_Wouters_2007"]["qme"] = BenchmarkGroup()

sol, qme_sol, solved = calculate_first_order_solution(∇₁; T = 𝓂.timings, opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :schur))

clear_solution_caches!(𝓂, :first_order)

SUITE["Smets_and_Wouters_2007"]["qme"]["schur"] =       @benchmarkable calculate_first_order_solution($∇₁; T = $𝓂.timings, opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :schur)) setup = clear_solution_caches!($𝓂, :first_order)
SUITE["Smets_and_Wouters_2007"]["qme"]["doubling"] =    @benchmarkable calculate_first_order_solution($∇₁; T = $𝓂.timings, opts = merge_calculation_options(quadratic_matrix_equation_algorithm = :doubling)) setup = clear_solution_caches!($𝓂, :first_order)
...
SUITE["Smets_and_Wouters_2007"]["lyapunov"] = BenchmarkGroup()
SUITE["Smets_and_Wouters_2007"]["lyapunov"]["doubling"] =           @benchmarkable solve_lyapunov_equation($A, $CC, lyapunov_algorithm = :doubling) # setup = clear_solution_caches!($𝓂, :first_order)
SUITE["Smets_and_Wouters_2007"]["lyapunov"]["bartels_stewart"] =    @benchmarkable solve_lyapunov_equation($A, $CC, lyapunov_algorithm = :bartels_stewart) # setup = clear_solution_caches!($𝓂, :first_order)
SUITE["Smets_and_Wouters_2007"]["lyapunov"]["bicgstab"] =           @benchmarkable solve_lyapunov_equation($A, $CC, lyapunov_algorithm = :bicgstab) # setup = clear_solution_caches!($𝓂, :first_order)
SUITE["Smets_and_Wouters_2007"]["lyapunov"]["gmres"] =              @benchmarkable solve_lyapunov_equation($A, $CC, lyapunov_algorithm = :gmres) # setup = clear_solution_caches!($𝓂, :first_order)

clear_solution_caches!(𝓂, :first_order)

SUITE["Smets_and_Wouters_2007"]["covariance"] = @benchmarkable get_covariance($𝓂) setup = clear_solution_caches!($𝓂, :first_order)
