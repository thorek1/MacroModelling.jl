...
Equivalent to `calculate_marginal_contribution_second_order` to within the
quadrature error (zero for the multilinear extension since the integrand is
a polynomial of degree ≤ 3 in `t`, integrated exactly by 2-node
Gauss–Legendre).

Cost: `n_nodes · nᵉ` Lyapunov solves with `n_nodes = 2`, versus
`Σⱼ≤4 C(nᵉ,j)` for the polynomial-coalition path. The two approaches
agree to machine precision on binary inputs and the AS path is always
cheaper for `nᵉ ≥ 4`.
"""
...
Aumann–Shapley driver for the pruned third-order variance decomposition.
...
`ŝ_to_ŝ₃`, giving `3·nᵉ` Lyapunov solves total versus
`Σⱼ≤min(6,nᵉ) C(nᵉ,j)` for the polynomial-coalition path
(`calculate_marginal_contribution_third_order`).

Returns `(shares::Matrix, total_var::Vector, ok::Bool)` analogously to
`calculate_marginal_contribution_third_order`.
"""
function calculate_aumann_shapley_third_order(parameters::Vector{R},
                                              𝓂::ℳ;
                                              opts::CalculationOptions = merge_calculation_options()
                                              )::Tuple{Matrix{R}, Vector{R}, Bool} where R <: Real
    Σʸ₃_total, _μʸ₂, _SS_and_pars, slvd_total = calculate_third_order_moments(parameters, :full_covar, 𝓂; opts = opts)
