...
macro model(𝓂,ex...)
    ...
    quote
        global $𝓂 =  ℳ(
                        ...
                        [
                                            # xtol ftol rel_xtol
                            solver_parameters(1e-12, 1e-14, eps(), 250, 
    ...
    return quote
        ...
        if !$precompile
            ...
            if solution_error > 1e-12
                # start_time = time()
                found_solution = find_SS_solver_parameters!(mod.$𝓂, verbosity = 0, maxtime = 120, maxiter = 10000000)
