...
function run_kalman_iterations(A::Matrix{S}, 
    ...
    for t in 1:size(data_in_deviations, 2)
        ...
        luF = fast_lu!(F, check = false) ### has to be LU since F will always be symmetric and positive semi-definite but not positive definite (due to linear dependencies)
        # end # timeit_debug
...
function filter_and_smooth(𝓂::ℳ, 
    ...
    for t in axes(data_in_deviations,2)
        ...
        F̄ = fast_lu(C * P[:, :, t] * C', 𝓂.workspaces, check = false)
