...
- `marginal_contribution` [Default: `false`, Type: `Bool`]: when `true` and `algorithm` is one of the pruned higher-order solutions, return marginal-contribution (Shapley-allocated) per-shock variance shares (via the Aumann–Shapley path-integral driver, `n_nodes * n_e` Lyapunov solves) instead of raw shares plus a `:Cross_shock_interaction` column. At `:first_order` the option is silently ignored (with an `@info` notice).
- $QME®
- $LYAPUNOV®
- $TOLERANCES®
- $VERBOSE®
