...
function get_conditional_forecast(𝓂::ℳ,
    ...
    if algorithm ∈ [:second_order, :third_order, :pruned_second_order, :pruned_third_order]
        ...
        initial_state = Stateupdate(Val(algorithm), initial_state::Vector{<:Real}, Float64[shocks[:,1]...], 𝓂.timings, 𝓂.solution.perturbation)
        ...
        for i in 2:size(conditions,2)
            ...
            initial_state = Stateupdate(Val(algorithm), initial_state::Vector{<:Real}, Float64[shocks[:,i]...], 𝓂.timings, 𝓂.solution.perturbation)
    ...
    elseif algorithm == :first_order
        ...
        shocks[free_shock_idx,1] = CC \ (conditions[cond_var_idx,1] - Stateupdate(Val(:first_order), initial_state::Vector{<:Real}, Float64[shocks[:,1]...], 𝓂.timings, 𝓂.solution.perturbation)[cond_var_idx])
        
        Y[:,1] = Stateupdate(Val(:first_order), initial_state::Vector{<:Real}, Float64[shocks[:,1]...], 𝓂.timings, 𝓂.solution.perturbation)
        ...
        for i in 2:size(conditions,2)
            ...
            shocks[free_shock_idx,i] = CC \ (conditions[cond_var_idx,i] - Stateupdate(Val(:first_order), (Y[:,i-1])::Vector{<:Real}, Float64[shocks[:,i]...], 𝓂.timings, 𝓂.solution.perturbation)[cond_var_idx])
    
            Y[:,i] = Stateupdate(Val(:first_order), (Y[:,i-1])::Vector{<:Real}, Float64[shocks[:,i]...], 𝓂.timings, 𝓂.solution.perturbation)
        end
    end
...
function get_irf(𝓂::ℳ,
    ...
    if shocks == :none
        ...
        push!(Y, Stateupdate(Val(:first_order), initial_state::Vector{<:Real}, shock_history[:,1], 𝓂.timings, 𝓂.solution.perturbation))
        
        for t in 1:periods-1
            push!(Y, Stateupdate(Val(:first_order), (Y[end])::Vector{<:Real}, shock_history[:,t+1], 𝓂.timings, 𝓂.solution.perturbation))
        end
