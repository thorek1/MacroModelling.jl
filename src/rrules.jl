...
function rrule(::typeof(solve_stochastic_steady_state_newton),
    ...
    for i in 1:max_iters
        ...
        compressed_kron²_power!(kron_x_aug_buf, x_aug)
    ...
    kron_x_aug = compressed_kron²_power(x_aug)
...
function rrule(::typeof(solve_stochastic_steady_state_newton),
    ...
    for i in 1:max_iters
        copyto!(x_aug, 1, x, 1, nPast)
        compressed_kron²_power!(kron_x_aug_buf, x_aug)
        compressed_kron³_power!(kron_x_kron_buf, x_aug)
    ...
    # Local kron for closure capture (workspace buffers may be overwritten before pullback runs)
    kron_x_aug = compressed_kron²_power(x_aug)
    kron_x_kron = compressed_kron³_power(x_aug)
...
function rrule(::typeof(prepare_stochastic_steady_state_base_terms),
    ...
    kron_aug1 = compressed_kron²_power(aug_state₁)
...
function rrule(::typeof(calculate_stochastic_steady_state),
    ...
    pullback = function (Δresult)
        ...
        kron_aug = compressed_kron²_power(aug_sss)
        ...
        # d/dx of compressed_kron²_power(aug(x)) is 2 * compressed_kron²(aug, ∂aug/∂x),
        # so the 1/2 in front of the 𝐒₂ term cancels. Same convention as the Newton
        # Jacobian in solve_stochastic_steady_state_newton.
        ∂SSSstates_from_state = A' * ∂state_vec
        dkron_dx = compressed_kron²(aug_sss, vcat(Matrix{Float64}(ℒ.I, nPast, nPast), zeros(1, nPast)))
        ∂SSSstates_from_state += (B̂' * ∂state_vec)' * dkron_dx |> vec
...
function rrule(::typeof(calculate_stochastic_steady_state),
    ...
    aug_sss = vcat(SSSstates_final, 1)
    kron_aug = compressed_kron²_power(aug_sss)
    kron_aug3 = compressed_kron³_power(aug_sss)
    ...
    pullback = function (Δresult)
        ...
        # Differentiating the compressed powers brings down their multiplicities:
        # d/dx compressed_kron²_power(aug) = 2 * compressed_kron²(aug, ∂aug/∂x) and
        # d/dx compressed_kron³_power(aug) = 3 * compressed_kron³(aug, aug, ∂aug/∂x),
        # so the 1/2 and 1/6 weights become 1 and 1/2. Same convention as the Newton
        # Jacobian in solve_stochastic_steady_state_newton.
        dkron_dx = compressed_kron²(aug_sss, pad)
        ∂SSSstates_from_state += (B̂' * ∂state_vec)' * dkron_dx |> vec

        dkron3_dx = compressed_kron³(aug_sss, aug_sss, pad)
        ∂SSSstates_from_state += (Ĉ' * ∂state_vec)' * dkron3_dx / 2 |> vec
...
function irf_bptt(::Val{:pruned_second_order},
    ...
    for si in 1:nShocks
        ...
        for t in periods:-1:1
            ...
            compressed_kron²_power!(kaug₁, aug₁)
            ...
            ∂𝐒₂ .+= ∂δ_t * kaug₁' / 2
            ∂kaug₁ = 𝐒₂' * ∂δ_t / 2
            compressed_kron²_power_vjp!(vjp_aug₁, ∂kaug₁, aug₁)
            ∂aug₁ .+= vjp_aug₁
...
function irf_bptt(::Val{:pruned_third_order},
    ...
    for si in 1:nShocks
        ...
        for t in periods:-1:1
            ...
            compressed_kron²_power_vjp!(vjp_aug₁, ∂kaug₁_from_δ, aug₁)
            ∂aug₁ .+= vjp_aug₁
...
function irf_bptt(::Val{:third_order},
    ...
    for si in 1:nShocks
        ...
        for t in periods:-1:1
            ...
            aug = [prev_st[past_idx]; one(S); shock_t]
            compressed_kron²_power!(kaug, aug)
            compressed_kron³_power!(kaug3, aug)
            ...
            compressed_kron²_power_vjp!(vjp_aug, ∂kaug, aug)
            ∂aug .+= vjp_aug
            compressed_kron³_power_vjp!(vjp_aug, ∂kaug3, aug)
            ∂aug .+= vjp_aug
...
function fill_kron_adjoint!(∂A::V, ∂B::V, ∂X::V, A::V, B::V) where V <: Vector{<: Real}
    ...
    if length(A) == length(B) && length(∂X) == length(A) * (length(A) + 1) ÷ 2
        ...
        pair_index = 0
        if ∂A === ∂B
            # Both argument cotangents share storage in the power-kernel
            # pullbacks.  Accumulate their sum without overwriting either
            # contribution or reading values after the cotangent is updated.
...
function rrule(::typeof(calculate_loglikelihood_with_missing), ::Val{:inversion}, ::Val{:pruned_second_order},
    ...
    pair_index_map = cc.compressed_pair_index_map
    ...
    if use_joint_warmup
        ...
        @inbounds for w in 1:warmup_iterations-1
            ...
            ℒ.mul!(state₂, 𝐒⁻¹, aug_state₂_seq[1])
            compressed_kron²_power!(kronaug_state₁, aug_state₁_seq[1])
    ...
    for t in 1:Tt
        ...
        ℒ.mul!(state₂, 𝐒⁻¹, aug_state₂_seq[t])
        compressed_kron²_power!(kronaug_state₁, aug_state₁_seq[t])
        ℒ.mul!(state₂, 𝐒⁻², kronaug_state₁, 1/2, 1)
    ...
    function pruned2_missing_pullback(∂llh)
        ...
        for t in Tt:-1:1
            ...
            ℒ.mul!(∂𝐒⁻¹, ∂state₂_next, aug_state₂', 1, 1)
            ℒ.mul!(∂aug_state₂, 𝐒⁻¹', ∂state₂_next)
            compressed_kron²_power!(kronaug_buf, aug_state₁)
            ...
            compressed_kron²_power!(kron_sv, state¹⁻_vol)
...
function second_order_warmup_state_pullback!(
    ...
    @inbounds for i in 1:warmup_iterations-1
        ...
        kronaug_state = compressed_kron²_power(aug_state)
...
function second_order_warmup_observation_and_jacobian_pullback!(
    ...
    kronstate_vol = compressed_kron²_power(state_vol)
    jac_state_vol = compressed_kron²(state_vol, I_state_vol)
    # 𝐒²⁻ᵉ multiplies a rectangular shock×state kron, not a symmetric pair, so
    # this stays a plain kron (as in second_order_warmup_observation_and_jacobian
    # and in the ∂𝐒²⁻ᵉ term below).
    ...
    ∂state_vol_pair = zeros(Float64, n_state_vol)
    ...
    compressed_kron²_power_vjp!(∂state_vol_pair, ∂kronstate_vol, state_vol)
    ∂state_vol .+= ∂state_vol_pair
...
function pruned_second_order_warmup_state_pullback!(
    ...
    @inbounds for i in 1:warmup_iterations-1
        ...
        kronaug_state1 = compressed_kron²_power(aug_state1)
...
function pruned_second_order_warmup_observation_and_jacobian_pullback!(
    ...
    final_shock = copy(view(warmup_shocks, :, n_warm))
    kronstate1_vol = compressed_kron²_power(state1_vol)
...
function pruned_third_order_warmup_state_pullback!(
    ...
    @inbounds for i in 1:warmup_iterations-1
        ...
        state1 = 𝐒⁻¹ * aug_state1
        state2 = 𝐒⁻¹ * aug_state2 + 𝐒⁻² * kronaug_state1 / 2
        state3 = 𝐒⁻¹ * aug_state3 + 𝐒⁻² * compressed_kron²(aug_state1hat, aug_state2) + 𝐒⁻³ * compressed_kron³_power(aug_state1) / 6
    ...
    @inbounds for i in warmup_iterations-1:-1:1
        ...
        kron_kron_aug_state1 = compressed_kron³_power(aug_state1)
        ℒ.mul!(∂𝐒⁻³, ∂state3, kron_kron_aug_state1', 1/6, 1)
        ∂kron_kron_aug_state1 = 𝐒⁻³' * ∂state3 / 6
        ∂triple_aug_state1 = zeros(Float64, n_aug)
        compressed_kron³_power_vjp!(∂triple_aug_state1, ∂kron_kron_aug_state1, aug_state1)
        ∂aug_state1 .+= ∂triple_aug_state1

        ℒ.mul!(∂𝐒⁻¹, ∂state2, aug_state2', 1, 1)
        ∂aug_state2_linear = 𝐒⁻¹' * ∂state2
        ∂aug_state2 .+= ∂aug_state2_linear

        ℒ.mul!(∂𝐒⁻², ∂state2, kronaug_state1', 1/2, 1)
        ∂kronaug_state1 = 𝐒⁻²' * ∂state2 / 2
        ∂pair_aug_state1 = zeros(Float64, n_aug)
        compressed_kron²_power_vjp!(∂pair_aug_state1, ∂kronaug_state1, aug_state1)
        ∂aug_state1 .+= ∂pair_aug_state1

        ℒ.mul!(∂𝐒⁻¹, ∂state1, aug_state1', 1, 1)
        ∂aug_state1_linear = 𝐒⁻¹' * ∂state1
        ∂aug_state1 .+= ∂aug_state1_linear
...
function rrule(::typeof(calculate_loglikelihood),
    ...
    if use_joint_warmup
        ...
        @inbounds for w in 1:warmup_iterations-1
            ...
            ℒ.mul!(state₂, 𝐒⁻¹, warmup_aug₂)
            compressed_kron²_power!(warmup_kronaug₁, warmup_aug₁)
    ...
    for i in axes(data_in_deviations,2)
        ...
        compressed_kron²_power!(kronxx[i], x[i])
        ...
        ℒ.mul!(state₂, 𝐒⁻¹, aug_state₂[i])
        compressed_kron²_power!(kronaug_state₁, aug_state₁[i])
        ℒ.mul!(state₂, 𝐒⁻², kronaug_state₁, 1/2, 1)
    end
...
function rrule(::typeof(calculate_loglikelihood_with_missing), ::Val{:inversion}, ::Val{:second_order},
    ...
    if use_joint_warmup
        ...
        @inbounds for w in 1:warmup_iterations-1
            ...
            compressed_kron²_power!(kronaug_state, aug_state_seq[1])
    ...
    for t in 1:Tt
        ...
        compressed_kron²_power!(kronaug_state, aug_state_seq[t])
        ℒ.mul!(st, 𝐒⁻², kronaug_state, 1/2, 1)
    ...
    function second_missing_pullback(∂llh)
        ...
        for t in Tt:-1:1
            ...
            ℒ.mul!(∂𝐒¹⁻ᵛ, ∂shock_independent, state¹⁻_vol', -1, 1)
            ℒ.mul!(∂state¹⁻_vol, 𝐒¹⁻ᵛ', ∂shock_independent, -1, 1)
            kron_sv = compressed_kron²_power(state¹⁻_vol)
            ℒ.mul!(∂𝐒²⁻ᵛ, ∂shock_independent, kron_sv', -1/2, 1)
            ∂kron_sv = -(𝐒²⁻ᵛ' * ∂shock_independent) ./ 2
            fill!(∂kronstate, 0)
            ∂kronstate .+= ∂kron_sv
...
function rrule(::typeof(calculate_loglikelihood),
    ...
    kronstate¹⁻_voltmp = compressed_kron²_power(state¹⁻_vol)
    ...
    @inbounds for i in axes(data_in_deviations,2)
        ...
        compressed_kron²_power!(kronstate¹⁻_vol[i], state¹⁻_vol)
        ...
        compressed_kron²_power!(kronaug_state[i], aug_state[i])
        ℒ.mul!(state¹⁻, 𝐒⁻¹, aug_state[i])
...
function rrule(::typeof(calculate_loglikelihood_with_missing), ::Val{:inversion}, ::Val{:pruned_third_order},
    ...
    if use_joint_warmup
        ...
        @inbounds for w in 1:warmup_iterations-1
            ...
            compressed_kron²_power!(warmup_kron_aug₁, warmup_aug₁)
            ℒ.mul!(state₁, 𝐒⁻¹, warmup_aug₁)
            ℒ.mul!(state₂, 𝐒⁻¹, warmup_aug₂)
            ...
            compressed_kron²_power!(warmup_kron_aug₁, warmup_aug₁)
            compressed_kron³_power!(warmup_kron_kron_aug₁, warmup_aug₁)
    ...
    for t in 1:Tt
        ...
        compressed_kron³_power!(kron_kron_state¹⁻_vol, state¹⁻_vol)
        ...
        if m == 0
            ...
            if t > presample_periods
                ...
                kron_J_x = kb3
                kron_xx  = compressed_kron²_power(x)
        ...
        compressed_kron²_power!(kron_aug_state₁, aug_state₁_seq[t])
        compressed_kron³_power!(kron_kron_aug_state₁, aug_state₁_seq[t])
    ...
    ∂aug_state₂ = zeros(n_past + 1 + n_exo)
    ∂aug_state₃ = zeros(n_past + 1 + n_exo)
    ∂aug_state₂_cross = zeros(n_past + 1 + n_exo)
    ∂kronstate  = zeros(n_state_pair)
    ∂state¹⁻_vol = zeros(n_past + 1)
    ∂state¹⁻_vol_pair = zeros(n_past + 1)
    ∂state¹⁻_vol_cubic = zeros(n_past + 1)
    ...
    function pruned3_missing_pullback(∂llh)
        ...
        for t in Tt:-1:1
            ...
            fill!(∂aug_state₁̂, 0)
            compressed_kron²_vjp!(∂aug_state₁̂,
                                  ∂aug_state₂_cross,
                                  ∂kronaug₁̂₂,
                                  aug_state₁̂,
                                  aug_state₂)
            ∂aug_state₂ .+= ∂aug_state₂_cross
            compressed_kron²_power!(kronaug_buf, aug_state₁)
            kron_kron_aug₁ = compressed_kron³_power(aug_state₁)
            ℒ.mul!(∂𝐒⁻³, ∂state₃_next, kron_kron_aug₁', 1/6, 1)
            ∂kronkronaug₁ = (𝐒⁻³' * ∂state₃_next) ./ 6
            fill!(∂aug_state₁, 0)
            compressed_kron³_power_vjp!(∂aug_state₁, ∂kronkronaug₁, aug_state₁)
            ...
            if m > 0
                ...
                kron_xx_local  = compressed_kron²_power(x)
            ...
            if m > 0
                ...
                @inbounds for j in 1:n_exo
                    ...
                    compressed_kron²_power_vjp!(∂state_pair, view(∂kron_u_s1v, rows, j), state¹⁻_vol)
            ...
            kron_s1v_3 = compressed_kron³_power(state¹⁻_vol)
            ...
            compressed_kron³_power_vjp!(∂state¹⁻_vol_cubic,
                                        ∂kron_s1v_3,
                                        state¹⁻_vol)
            compressed_kron²_power_vjp!(∂state¹⁻_vol_pair,
                                        ∂kronstate,
                                        state¹⁻_vol)
            ∂state¹⁻_vol .+= ∂state¹⁻_vol_cubic
            ∂state¹⁻_vol .+= ∂state¹⁻_vol_pair
...
function rrule(::typeof(calculate_loglikelihood),
    ...
    for i in axes(data_in_deviations,2)
        ...
        ℒ.mul!(shock_independent, 𝐒³⁻ᵛ, compressed_kron³_power(state¹⁻_vol), -1/6, 1)
        ...
        compressed_kron³_power!(kronxxx[i], x[i])
        ...
        compressed_kron²_power!(kron_aug_state₁[i], aug_state₁[i])

        state₁, state₂, state₃ = [𝐒⁻¹ * aug_state₁[i], 𝐒⁻¹ * aug_state₂[i] + 𝐒⁻² * kron_aug_state₁[i] / 2, 𝐒⁻¹ * aug_state₃[i] + 𝐒⁻² * compressed_kron²(aug_state₁̂[i], aug_state₂[i]) + 𝐒⁻³ * compressed_kron³_power(aug_state₁[i]) / 6]
    end
    # end # timeit_debug
    ...
    ∂aug_state₂_cross = zeros(size(∂aug_state₁̂))
    ∂aug_state₁_cubic = zeros(size(∂aug_state₁̂))
    ∂aug_state₁_pair  = zeros(size(∂aug_state₁̂))
    ...
    kronstate¹⁻_vol_buf = zeros(n_state_pair)
    ...
    ∂state_pair_left = zeros(T.nPast_not_future_and_mixed)
    ...
    function inversion_filter_loglikelihood_pullback(∂llh)
        ...
        for i in reverse(axes(data_in_deviations,2))
            ...
            ∂𝐒⁻³ += ∂state[3] * compressed_kron³_power(aug_state₁[i])' / 6

            ∂kronkronaug_state₁ = 𝐒⁻³' * ∂state[3] / 6

            compressed_kron³_power_vjp!(∂aug_state₁_cubic,
                                        ∂kronkronaug_state₁,
                                        aug_state₁[i])
            ∂aug_state₁ .+= ∂aug_state₁_cubic
    
            # kron_aug_state₁[i] = compressed_kron²_power(aug_state₁[i])
            compressed_kron²_power_vjp!(∂aug_state₁_pair, ∂kronaug_state₁, aug_state₁[i])
            ∂aug_state₁ .+= ∂aug_state₁_pair
            ...
            triple_xxS1 = compressed_kron³(x[i], x[i], S1)
            ℒ.mul!(∂𝐒ⁱ³ᵉ, λ[i], triple_xxS1', 3, 1)
            ℒ.mul!(∂𝐒ⁱ³ᵉ, S2, kronxxx[i]', -1, 1)
            ...
            # Forward value of the state-vol pair; must not touch the
            # ∂kronstate¹⁻_vol cotangent, which is still being accumulated.
            compressed_kron²_power!(kronstate¹⁻_vol_buf, state¹⁻_vol)
            ℒ.kron!(kron_buffer3sv, J, kronstate¹⁻_vol_buf)
            ∂𝐒³⁻ᵉ² += ∂𝐒ⁱ * kron_buffer3sv' / 2
            ∂kronIstate¹⁻_volstate¹⁻_vol = 𝐒³⁻ᵉ²' * ∂𝐒ⁱ / 2
            ∂state_pair = zeros(n_state_vol)
            @inbounds for j in 1:T.nExo
                rows = (j - 1) * n_state_pair + 1:j * n_state_pair
                compressed_kron²_power_vjp!(∂state_pair, view(∂kronIstate¹⁻_volstate¹⁻_vol, rows, j), state¹⁻_vol)
                ∂state¹⁻_vol .+= ∂state_pair
            end
            ...
            compressed_kron²_vjp!(∂state_pair_left,
                                  ∂state_pair_right,
                                  ∂kronstate¹⁻²⁻,
                                  state¹⁻,
                                  state²⁻)
            ∂state[1] .+= ∂state_pair_left
            ∂state[2] .+= ∂state_pair_right

            # ℒ.mul!(shock_independent, 𝐒³⁻ᵛ, ℒ.kron(state¹⁻_vol, ℒ.kron(state¹⁻_vol, state¹⁻_vol)), -1/6, 1)   
            ∂𝐒³⁻ᵛ -= ∂shock_independent * compressed_kron³_power(state¹⁻_vol)' / 6

            ∂kronstate¹⁻_volstate¹⁻_vol = -𝐒³⁻ᵛ' * ∂shock_independent / 6

            ∂state_cubic = zeros(n_state_vol)
            compressed_kron³_power_vjp!(∂state_cubic, ∂kronstate¹⁻_volstate¹⁻_vol, state¹⁻_vol)
            ∂state¹⁻_vol .+= ∂state_cubic
            ∂state_pair = zeros(n_state_vol)
            compressed_kron²_power_vjp!(∂state_pair, ∂kronstate¹⁻_vol, state¹⁻_vol)
            ∂state¹⁻_vol .+= ∂state_pair
...
function rrule(::typeof(calculate_loglikelihood_with_missing), ::Val{:inversion}, ::Val{:third_order},
    ...
    for t in 1:Tt
        ...
        compressed_kron²_power!(kron_aug_state, aug_state_seq[t])
        compressed_kron³_power!(kron_kron_aug_state, aug_state_seq[t])
        ℒ.mul!(st, 𝐒⁻¹, aug_state_seq[t])
        ℒ.mul!(st, 𝐒⁻², kron_aug_state, 1/2, 1)
        ℒ.mul!(st, 𝐒⁻³, kron_kron_aug_state, 1/6, 1)
        copyto!(st_seq[t+1], st)
    ...
    ∂aug_state  = zeros(n_past + 1 + n_exo)
    ∂aug_state_pair_third = zeros(n_past + 1 + n_exo)
    ∂aug_state_cubic_third = zeros(n_past + 1 + n_exo)
    ∂kronstate  = zeros(n_state_vol * (n_state_vol + 1) ÷ 2)
    ∂state¹⁻_vol = zeros(n_past + 1)
    ∂state¹⁻_vol_pair_third = zeros(n_past + 1)
    ∂state¹⁻_vol_cubic_third = zeros(n_past + 1)
    ∂𝐒ⁱ_full_buf      = zeros(length(cond_var_idx), n_exo)
    ...
    function third_order_missing_pullback(∂llh)
        ...
        for t in Tt:-1:1
            ...
            # kronstate¹⁻_vol is a shared workspace buffer, so it still holds the
            # value from the last forward step. Refresh it for this period before
            # it is read back below.
            compressed_kron²_power!(kronstate¹⁻_vol, state¹⁻_vol)
            ...
            compressed_kron³_power_vjp!(∂aug_state_cubic_third, ∂kronkronaug, aug_state)
            compressed_kron²_power_vjp!(∂aug_state_pair_third, ∂kronaug2, aug_state)
            ∂aug_state .+= ∂aug_state_cubic_third
            ∂aug_state .+= ∂aug_state_pair_third
            ...
            if m > 0
                ...
                xSx = x * Sx'
                xx_outer = x * x'
                ∂𝐒ⁱ²ᵉ_v_kkt = 2 * λ * (compressed_kron²(x, Sx))' - Sλ * (compressed_kron²_power(x))'
                ∂𝐒ⁱ³ᵉ_v_kkt = 3 * λ * (compressed_kron³(x, x, Sx))' - Sλ * (compressed_kron³_power(x))'
            ...
            ℒ.mul!(∂𝐒³⁻ᵛ, ∂shock_independent, kron_s1v_3', -1/6, 1)
            ∂kron_s1v_3 = -(𝐒³⁻ᵛ' * ∂shock_independent) ./ 6
            compressed_kron³_power_vjp!(∂state¹⁻_vol_cubic_third,
                                        ∂kron_s1v_3,
                                        state¹⁻_vol)
            compressed_kron²_power_vjp!(∂state¹⁻_vol_pair_third,
                                        ∂kron_sv,
                                        state¹⁻_vol,
                                        1)
            ∂state¹⁻_vol .+= ∂state¹⁻_vol_cubic_third
            ∂state¹⁻_vol .+= ∂state¹⁻_vol_pair_third
...
function accumulate_cubic_kron_jacobian_pullback!(
    ...
    # compressed_kron³_identity_vjp! is already the exact VJP of
    # compressed_kron³(vec, vec, I); the caller scales ∂jac_term, so no extra weight.
    compressed_kron³_identity_vjp!(∂vec, ∂jac_term, vec)
...
function third_order_warmup_state_pullback!(
    ...
    @inbounds for i in 1:warmup_iterations-1
        aug_state = [st; 1.0; view(warmup_shocks, :, i)]
        kronaug_state = compressed_kron²_power(aug_state)
        kronkronaug_state = compressed_kron³_power(aug_state)
    ...
    @inbounds for i in warmup_iterations-1:-1:1
        ...
        ∂aug_state_cubic = similar(∂aug_state)
        ∂aug_state_pair = similar(∂aug_state)
        compressed_kron³_power_vjp!(∂aug_state_cubic, ∂kronkronaug_state, aug_state)
        compressed_kron²_power_vjp!(∂aug_state_pair, ∂kronaug_state, aug_state)
        ∂aug_state .+= ∂aug_state_cubic
        ∂aug_state .+= ∂aug_state_pair
...
function third_order_warmup_observation_and_jacobian_pullback!(
    ...
    final_shock = copy(view(warmup_shocks, :, n_warm))
    kronstate_vol = compressed_kron²_power(state_vol)
    kronstate_vol3 = compressed_kron³_power(state_vol)
    kron_shock_state = ℒ.kron(final_shock, state_vol)
    kron_shock_shock = compressed_kron²_power(final_shock)
    ...
    kron_shock2_state = compressed_triple_shock_shock_state(
        final_shock, state_vol, shock_offset, shock_shock_state_indices;
        index_rows = shock_shock_state_rows)
    kron_shock3 = compressed_kron³_power(final_shock)
    ...
    ∂state_vol = zeros(Float64, n_state_vol)
    ∂state_vol_pair = zeros(Float64, n_state_vol)
    ∂state_vol_cubic = zeros(Float64, n_state_vol)
    ∂final_shock = zeros(Float64, n_exo)
    ∂final_shock_pair = zeros(Float64, n_exo)
    ∂final_shock_cubic = zeros(Float64, n_exo)
    ∂ds_dz = zeros(Float64, n_past, n_z)
    ...
    ℒ.mul!(∂𝐒²⁻ᵛ, ∂y_pred, kronstate_vol', 1/2, 1)
    ∂kronstate_vol = 𝐒²⁻ᵛ' * ∂y_pred / 2
    # compressed_kron²_power_vjp! overwrites its output, so it needs the scratch
    # vector; writing straight into ∂state_vol would drop the 𝐒¹⁻ᵛ term above.
    compressed_kron²_power_vjp!(∂state_vol_pair, ∂kronstate_vol, state_vol)
    ∂state_vol .+= ∂state_vol_pair

    ℒ.mul!(∂𝐒³⁻ᵛ, ∂y_pred, kronstate_vol3', 1/6, 1)
    ∂kronstate_vol3 = 𝐒³⁻ᵛ' * ∂y_pred / 6
    compressed_kron³_power_vjp!(∂state_vol_cubic, ∂kronstate_vol3, state_vol)
    ∂state_vol .+= ∂state_vol_cubic
    ...
    ℒ.mul!(∂𝐒²ᵉ, ∂y_pred, kron_shock_shock', 1/2, 1)
    ∂kron_shock_shock = 𝐒²ᵉ' * ∂y_pred / 2
    compressed_kron²_power_vjp!(∂final_shock_pair, ∂kron_shock_shock, final_shock)
    ∂final_shock .+= ∂final_shock_pair
    ...
    compressed_kron³_power_vjp!(∂final_shock_cubic, ∂kron_shock3, final_shock)
    ∂final_shock .+= ∂final_shock_cubic

    ∂jac_y_state = zeros(Float64, n_obs, n_state_vol)
    if n_past > 0
        @views ∂jac_y_state[:, 1:n_past] .+= ∂jac_seed * ds_dz'
        ℒ.mul!(∂ds_dz, view(𝐒¹⁻ᵛ, :, 1:n_past)', zeros(Float64, n_obs, n_z), 0, 0)
        # d/ds of 𝐒³⁻ᵛ·compressed_kron³_power(s)/6 is 𝐒³⁻ᵛ·compressed_kron³(s,s,I)/2,
        # matching third_order_warmup_observation_and_jacobian.
        jac_y_state = 𝐒¹⁻ᵛ + 𝐒²⁻ᵛ * jac_state2_term + 𝐒³⁻ᵛ * (jac_state3_term / 2)
    ...
    # jac_y_state carries 𝐒³⁻ᵛ · jac_state3_term / 2, so both cotangents get the 1/2.
    ℒ.mul!(∂𝐒³⁻ᵛ, ∂jac_y_state, jac_state3_term', 1/2, 1)
    ∂jac_state3_term = (𝐒³⁻ᵛ' * ∂jac_y_state) ./ 2
    ...
    @inbounds for i in n_warm-1:-1:1
        ...
        # The *_power_vjp! helpers overwrite their output, so they need scratch
        # vectors rather than the shared ∂aug_state accumulator.
        ∂aug_state_cubic = zeros(Float64, n_aug)
        compressed_kron³_power_vjp!(∂aug_state_cubic, ∂kronkronaug_state, aug_state)
        ∂aug_state .+= ∂aug_state_cubic
        ∂aug_state_pair = zeros(Float64, n_aug)
        compressed_kron²_power_vjp!(∂aug_state_pair, ∂kronaug_state, aug_state)
        ∂aug_state .+= ∂aug_state_pair
...
function rrule(::typeof(calculate_loglikelihood),
    ...
    #
    # `obs_idx_per_t` holds *positions within the observable set* (rows of
    # `data_in_deviations` and of the `cond_var_idx`-sliced solution matrices),
    # not the global variable indices in `observables_index`. With complete data
    # every period observes all of them, so the entry is `1:n_cond`.
    all_obs = collect(1:length(observables_index))
    obs_idx_per_t = [copy(all_obs) for _ in axes(data_in_deviations, 2)]
    ...
    function inversion_filter_loglikelihood_pullback(∂llh)
        ...
        for i in reverse(axes(data_in_deviations,2))
            ...
            triple_xxx = compressed_kron³_power(x[i])
...
function accumulate_symmetric_kron_pullback!(d_aug::AbstractVector, d_kron::AbstractVector, aug::AbstractVector)
    ...
    compressed_kron²_power_vjp!(contribution, d_kron, aug)
    d_aug .+= contribution
    return d_aug
end
...
function quad_adjoint(𝐒₂, aug::AbstractVector, d_new_state::AbstractVector)
    ...
    kaa = compressed_kron²_power(aug)
...
function filter_free_pullback_3rd(
    ...
    @inbounds for t in nT:-1:1
        ...
        compressed_kron³_power_vjp!(d_aug_cubic, d_kaug3, aug)
        d_aug .+= d_aug_cubic
...
function filter_free_pullback_pruned3rd(
    ...
    @inbounds for t in nT:-1:1
        ...
        compressed_kron³_power_vjp!(d_aug₁_cubic, d_kaug3, aug₁)
        d_aug₁ .+= d_aug₁_cubic
...
function rrule(::typeof(get_loglikelihood),
    ...
    elseif algorithm == :third_order
        ...
        @inbounds for t in 1:n_warm
            aug = vcat(cur_state[past_in_needed], one(R), Vector{R}(aligned_shocks[:, t]))
            kaug = compressed_kron²_power(aug)
            kaug3 = compressed_kron³_power(aug)
        ...
        @inbounds for t in 1:n_warm
            ...
            cur_state = [𝐒₁ * aug₁,
                         𝐒₁ * aug₂ + (𝐒₂ * kaug₁) ./ R(2),
                         𝐒₁ * aug₃ + 𝐒₂ * compressed_kron²(aug₁̂, aug₂) + (𝐒₃ * compressed_kron³_power(aug₁)) ./ R(6)]
        end
        @inbounds for t in 1:nT
            idx = obs_idx_per_t[t]
            ϵ = Vector{R}(aligned_shocks[:, n_warm + t])
            ...
            new3 = 𝐒₁ * aug₃ + 𝐒₂ * compressed_kron²(aug₁̂, aug₂) + (𝐒₃ * compressed_kron³_power(aug₁)) ./ R(6)
