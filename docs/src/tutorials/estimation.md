...
In order to make the prior definitions more intuitive for users we provide helper function translating the mean and standard deviation of the beta, inverse gamma, and gamma distributions to the respective distribution parameters (e.g. α and β for the beta distribution).
```@repl tutorial_2
...
We use the NUTS sampler to retrieve the posterior distribution of the parameters. This sampler uses the gradient of the posterior loglikelihood with respect to the model parameters to navigate the parameter space. The NUTS sampler is considered robust, fast, and has the advantage of auto-tuning.

First we define the turing model referring to the previously defined Turing model, and then we sample from the model:
```@repl tutorial_2
turing_model = kalman(data, FS2000, observables)
n_samples = 1000
chain_NUTS  = sample(turing_model, NUTS(), n_samples, progress = false)
```
...
Next, we setup the optimzation problem and first use the ADAM global optimiser for 1000 iterations in order to avoid local optima and then fine tune with L-BFGS.
```@repl tutorial_2
