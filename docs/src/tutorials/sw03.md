...
Under a pruned higher-order solution (`algorithm = :pruned_second_order` or `:pruned_third_order`) the table additionally contains a `:Cross_shock_interaction` column collecting variance generated by genuine products of distinct shocks in the centered higher moments. Setting `marginal_contribution = true` instead allocates that interaction across the individual shocks via marginal contributions (Shapley values), so the returned table has one column per shock and rows that still sum to one. The option is silently ignored at first order (where the decomposition is already additive across shocks).
