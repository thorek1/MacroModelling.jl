...
println("--- Forward primal (missing 3rd) ---")
ll_nan = get_loglikelihood(FS2000, data_nan, FS2000.parameter_values; algorithm = :third_order, filter = :inversion)
println("ll_nan = ", ll_nan)
@assert isfinite(ll_nan)

g_zy = nothing
println("--- Zygote pullback (3rd missing) ---")
f = x -> get_loglikelihood(FS2000, data_nan, x; algorithm = :third_order, filter = :inversion)
g_zy = nothing
g_zy = nothing
try
