...
println("--- Forward primal (missing 2nd non-pruned) ---")
ll_nan = get_loglikelihood(FS2000, data_nan, FS2000.parameter_values; algorithm = :second_order, filter = :inversion)
println("ll_nan = ", ll_nan)
@assert isfinite(ll_nan)

g_zy = nothing
println("--- Zygote pullback (2nd non-pruned missing) ---")
f = x -> get_loglikelihood(FS2000, data_nan, x; algorithm = :second_order, filter = :inversion)
g_zy = nothing
g_zy = nothing
try
