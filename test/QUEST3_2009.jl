...
@model QUEST3_2009 begin
	...
	inflation[0] = 0.25 * (inflationq[0] + inflationq[-1] + AUX_ENDO_LAG_104_1[-1] + AUX_ENDO_LAG_104_2[-1])
	...
	E_VL[0] = exp(E_ZEPS_C[0]) * OMEGE * exp(E_ZEPS_L[0]) * KAPPAE * (exp(E_LCNLCSN[0]) * (1 - HABE / (1 + E_GCNLC[0] - GY0))) ^ (1 - SIGC) * (1 - OMEGE * exp(E_ZEPS_L[0]) * (exp(E_LL[0]) - HABLE * exp(E_LL[-1])) ^ KAPPAE) ^ (-SIGC) * (exp(E_LL[0]) - HABLE * exp(E_LL[-1])) ^ (KAPPAE - 1)

	E_VLLC[0] = (1 - OMEGE * exp(E_ZEPS_L[0]) * (exp(E_LL[0]) - HABLE * exp(E_LL[-1])) ^ KAPPAE) ^ (-SIGC) * KAPPAE * OMEGE * exp(E_ZEPS_L[0]) * (exp(E_LL[0]) - HABLE * exp(E_LL[-1])) ^ (KAPPAE - 1) * exp(E_LCLCSN[0]) ^ (1 - SIGC)
	...
	(1 + TVAT) * ((E_VL[0] * (1 - SLC) + E_VLLC[0] * SLC) / (exp(E_LUCYN[0]) * (1 - SLC) + exp(E_LUCLCYN[0]) * SLC)) ^ (1 - WRLAG) * ((1 - E_TW[0] - SSC) / (1 + TVAT) * (THETAE - 1) / THETAE / exp(E_LYWR[-1]) / (1 + E_GY[0] - GY0)) ^ WRLAG = (1 - E_TW[0] - SSC) * (THETAE - 1) / THETAE / exp(E_LYWR[0]) + GAMIFLAG * GAMWE / THETAE / exp(E_LYWR[0]) * (E_WPHI[0] - GP0 - GY0 - (1 - SFWE) * (E_PHI[-1] - GP0)) - GAMIFLAG * BETAE * GAMWE / THETAE / exp(E_LYWR[0]) * (E_WPHI[1] - GP0 - GY0 - (1 - SFWE) * (E_PHI[0] - GP0))

	(1 + E_ZEPS_W[0]) / exp(E_LYWR[0]) = E_ETA[0] * ALPHAE / exp(E_LL[0]) * (1 + E_LOL[0]) - 1 / exp(E_LYWR[0]) * GAMLE * (E_LL[0] - E_LL[-1]) + GAMLE / exp(E_LYWR[1]) * (1 + E_GY[1] - GY0) / (1 + E_R[0]) * (E_LL[1] - E_LL[0])

	GAMIE * (exp(E_LIK[0]) - (GY0 + DELTAE + GPOP0 + GPCPI0)) + GAMI2E * (E_GI[0] - GY0 - GPCPI0) - GAMI2E / (1 + E_INOM[0]) * (E_GI[1] - GY0 - GPCPI0) = E_Q[0] - 1

	E_ETA[0] * (1 - TP) * (1 - ALPHAE) * exp(E_LYKPPI[0]) = E_Q[0] - (1 + GPCPI0 - E_R[0] - DELTAE - RPREMK - E_ZEPS_RPREMK[0] - E_PHIPI[1]) * E_Q[1] + (1 - TP) * (A1E * (E_UCAP[0] - UCAP0) + A2E * (E_UCAP[0] - UCAP0) ^ 2)

	(1 - ALPHAE) * E_ETA[0] * exp(E_LYKPPI[0]) = E_UCAP[0] * (A1E + A2E * 2 * (E_UCAP[0] - UCAP0))
	...
	E_GKG[0] - (GY0 + GPCPI0) = exp(E_LIKG[0]) - (GPOP0 + GY0 + GPCPI0 + DELTAGE)

	E_LISN[0] = E_LIK[0] + GY0 + GPCPI0 - E_LYKPPI[0] - E_GK[0]
	...
	exp(E_LIMYN[0]) = (1 - SE) * (exp(E_LCSN[0]) + exp(E_LISN[0]) + exp(E_LIGSN[0]) + exp(E_LGSN[0])) * exp(RHOPCPM * (E_LPCP[-1] - E_LPMP[-1]) + (1 - RHOPCPM) * (E_LPCP[0] - E_LPMP[0])) ^ SIGIME * exp(E_LPMP[0] - E_LPCP[0])

	exp(E_LEXYN[0]) = (1 - SE) * exp(E_LPXP[0]) * exp(RHOPWPX * (ALPHAX * SE * E_LER[-1] - E_LPXP[-1]) + (1 - RHOPWPX) * (ALPHAX * SE * E_LER[0] - E_LPXP[0])) ^ SIGEXE * exp(E_LYWY[0]) ^ ALPHAX
	...
	E_ZEPS_PPI[0] = STD_EPS_PPI * E_EPS_PPI[x] + RHOPPI1 * E_ZEPS_PPI[-1] + RHOPPI2 * AUX_ENDO_LAG_98_1[-1] + RHOPPI3 * AUX_ENDO_LAG_98_2[-1] + RHOPPI4 * AUX_ENDO_LAG_98_3[-1]
	...
	E_GY[0] + E_PHIPI[0] - E_GK[0] = E_LYKPPI[0] - E_LYKPPI[-1]
	...
	E_GY[0] + E_PHI[0] - E_WPHI[0] = E_LYWR[0] - E_LYWR[-1]
	...
	AUX_ENDO_LAG_104_1[0] = inflationq[-1]

	AUX_ENDO_LAG_104_2[0] = AUX_ENDO_LAG_104_1[-1]

	AUX_ENDO_LAG_98_1[0] = E_ZEPS_PPI[-1]

	AUX_ENDO_LAG_98_2[0] = AUX_ENDO_LAG_98_1[-1]

	AUX_ENDO_LAG_98_3[0] = AUX_ENDO_LAG_98_2[-1]
