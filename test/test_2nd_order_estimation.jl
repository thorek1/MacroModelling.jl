...
@testset "Mooncake vs FiniteDifferences gradient (2nd order, missing data)" begin
    back_grad = DifferentiationInterface.gradient(x -> get_loglikelihood(FS2000, data_missing, x, algorithm = :second_order), ADTypes.AutoMooncake(config = nothing), FS2000.parameter_values)
    @test !isnothing(back_grad)
    @test all(isfinite, back_grad)
