...
s = ParameterSchedulers.Stateful(CosAnneal(.001, 1e-6, 1000))
...
for i in 1:10
    ...
    if normalise
        ...
        sim_slices = Float32.(vcat(normalised_sims[:,1:end - 1], shcks[:,n_burnin + 1:n_burnin + n_simul]))

        out_slices = Float32.(normalised_sims[:,2:end])
    else
    ...
    for epoch in 1:2000
        lss, grads = Flux.withgradient(neural_net) do nn
            sqrt(Flux.mse(nn(sim_slices), out_slices))
        end
...
norm(out_slices - neural_net(sim_slices)) / norm(out_slices)