@model Backus_Kehoe_Kydland_1992_test begin
    for co in [H, F]
        ...
        pp{co}[0] = for lag2 in 0:(4-2)
                        for lag in 0:(4-1)  
                            if lag >= lag2
                                beta{co}^(lag) * LGM[lag]*phi{co}
                            elseif lag2 < 2
                                beta{co}^(lag+3) * LGM[lag2]*phi{co}
                            else
                                beta{co}^(lag2+1) * LGM[lag]*phi{co}
                            end
                        end
                    end
    ...
    # for lag2 in 0:(4-2)
        ...
        if co == H
            NX{F}{H}[0] = (NX{F}[0] - NX{H}[0])
        elseif co == F
            NX{H}{F}[0] = (NX{H}[0] - NX{F}[0])
        # else
        #     NX{H}{F}[0] = (NX{H}[0] - NX{F}[0])
        end

        if co == H
            NX3{F}{H}[0] = (NX{F}[0] - NX{H}[0])
        # elseif co == F
        #     NX3{H}{F}[0] = (NX{H}[0] - NX{F}[0])
        # else
        #     NX{H}{F}[0] = (NX{H}[0] - NX{F}[0])
        end

        co == H ? NX2{F}{H}[0] = (NX{F}[0] - NX{H}[0]) : NX2{H}{F}[0] = (NX{H}[0] - NX{F}[0]) 
    end
    ...
    for co2 in [HH,FF] 
        for co in [H,F] 
            if co == H
                NX{co2}{F}{H}[0] = (NX{F}[0] - NX{H}[0])
            else
                NX{co2}{H}{F}[0] = (NX{H}[0] - NX{F}[0]) 
            end
        end
    end
end

@parameters Backus_Kehoe_Kydland_1992_test begin
    K_ss = 11.0148
    # K[ss] = K_ss | beta
    # K[ss] = 10 | beta
    F_H_ratio = 1
