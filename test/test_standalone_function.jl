...
@testset verbose = true "Higher order IRFs and GIRFs" begin
    ...
    pruned_second_order_state_update = function(pruned_states::Vector{Vector{Float64}}, shock::Vector{Float64})
        aug_state₁ = [pruned_states[1][RBC_CME.constants.timings.past_not_future_and_mixed_idx]; 1; shock]
        aug_state₂ = [pruned_states[2][RBC_CME.constants.timings.past_not_future_and_mixed_idx]; 0; zero(shock)]
    ...
    pruned_third_order_state_update = function(pruned_states::Vector{Vector{Float64}}, shock::Vector{Float64})
        aug_state₁ = [pruned_states[1][RBC_CME.constants.timings.past_not_future_and_mixed_idx]; 1; shock]
        aug_state₁̂ = [pruned_states[1][RBC_CME.constants.timings.past_not_future_and_mixed_idx]; 0; shock]
        aug_state₂ = [pruned_states[2][RBC_CME.constants.timings.past_not_future_and_mixed_idx]; 0; zero(shock)]
        aug_state₃ = [pruned_states[3][RBC_CME.constants.timings.past_not_future_and_mixed_idx]; 0; zero(shock)]
    ...
    SSS_delta = RBC_CME.solution.non_stochastic_steady_state[1:length(RBC_CME.constants.timings.var)] - RBC_CME.solution.perturbation.second_order.stochastic_steady_state

    initial_state = zeros(RBC_CME.constants.timings.nVars) - SSS_delta
    ...
    SSS_delta = RBC_CME.solution.non_stochastic_steady_state[1:length(RBC_CME.constants.timings.var)] - RBC_CME.solution.perturbation.third_order.stochastic_steady_state
    ...
    SSS_delta = RBC_CME.solution.non_stochastic_steady_state[1:length(RBC_CME.constants.timings.var)] - RBC_CME.solution.perturbation.pruned_second_order.stochastic_steady_state

    initial_state = [zeros(RBC_CME.constants.timings.nVars), zeros(RBC_CME.constants.timings.nVars) ]
    ...
    SSS_delta = RBC_CME.solution.non_stochastic_steady_state[1:length(RBC_CME.constants.timings.var)] - RBC_CME.solution.perturbation.pruned_third_order.stochastic_steady_state

    initial_state = [zeros(RBC_CME.constants.timings.nVars), zeros(RBC_CME.constants.timings.nVars), zeros(RBC_CME.constants.timings.nVars)]
...
@testset verbose = true "NSSS and std derivatives" begin
    ...
    stdev_deriv = ForwardDiff.jacobian(x -> get_statistics(RBC_CME, x, parameters = RBC_CME.parameters, standard_deviation = RBC_CME.constants.timings.var)[:standard_deviation], RBC_CME.parameter_values)
    
    @test isapprox(stdev_deriv[5,6],1.3135107627695757, rtol = 1e-6)

    # derivatives of paramteres wrt non-stochastic steady state
    nsss_deriv = ForwardDiff.jacobian(x -> get_statistics(RBC_CME, x, parameters = RBC_CME.parameters, non_stochastic_steady_state = RBC_CME.constants.timings.var)[:non_stochastic_steady_state], RBC_CME.parameter_values)
...
@testset verbose = true "Method of moments" begin
    ...
    sol = Optim.optimize(x -> sum(abs2, get_statistics(RBC_CME, x, parameters = [RBC_CME.parameters[1]], standard_deviation = [RBC_CME.constants.timings.var[5]])[:standard_deviation] - [.21]),
    [0], [1], [.16], 
    Optim.Fminbox(Optim.LBFGS(linesearch = LineSearches.BackTracking(order = 3))); autodiff = :forward)

    @test isapprox(get_statistics(RBC_CME, sol.minimizer, parameters = [RBC_CME.parameters[1]], standard_deviation = [RBC_CME.constants.timings.var[5]])[:standard_deviation] ,[.21], atol = 1e-6)

    # pruned second order
    sol = Optim.optimize(x -> sum(abs2, get_statistics(RBC_CME, x, parameters = [RBC_CME.parameters[1]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_second_order)[:standard_deviation] - [.21]),
    [0], [1], [.16], 
    Optim.Fminbox(Optim.LBFGS(linesearch = LineSearches.BackTracking(order = 3))); autodiff = :forward)

    @test isapprox(get_statistics(RBC_CME, sol.minimizer, parameters = [RBC_CME.parameters[1]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_second_order)[:standard_deviation] ,[.21], atol = 1e-6)

    # pruned third order
    sol = Optim.optimize(x -> sum(abs2, get_statistics(RBC_CME, x, parameters = [RBC_CME.parameters[1]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_third_order)[:standard_deviation] - [.21]),
    [0], [1], [.16], 
    Optim.Fminbox(Optim.LBFGS(linesearch = LineSearches.BackTracking(order = 3))); autodiff = :forward)

    @test isapprox(get_statistics(RBC_CME, sol.minimizer, parameters = [RBC_CME.parameters[1]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_third_order)[:standard_deviation] ,[.21], atol = 1e-6)


    # multiple parameter inputs and targets
    sol = Optim.optimize(x -> sum(abs2,get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[[6,1]], standard_deviation = RBC_CME.constants.timings.var[[2,5]])[:standard_deviation] - [.0008,.21]),
    [0,0], [1,1], [.5,.16], 
    ...
    @test isapprox(get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[[6,1]], standard_deviation = RBC_CME.constants.timings.var[[2,5]])[:standard_deviation], [.0008,.21], atol=1e-6)

    # pruned second order
    sol = Optim.optimize(x -> sum(abs2,get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[[6,1]], standard_deviation = RBC_CME.constants.timings.var[[2,5]], algorithm = :pruned_second_order)[:standard_deviation] - [.0008,.21]),
    [0,0], [1,1], [.5,.16], 
    Optim.Fminbox(Optim.LBFGS(linesearch = LineSearches.BackTracking(order = 3))),
    # Optim.Options(show_trace = true,
    # store_trace = true,
    ...
    @test isapprox(get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[[6,1]], standard_deviation = RBC_CME.constants.timings.var[[2,5]], algorithm = :pruned_second_order)[:standard_deviation], [.0008,.21], atol=1e-6)

    # pruned third order
    sol = Optim.optimize(x -> sum(abs2,get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[[6,1]], standard_deviation = RBC_CME.constants.timings.var[[2,5]], algorithm = :pruned_third_order)[:standard_deviation] - [.0008,.21]),
    [0,0], [1,1], [.5,.16], 
    Optim.Fminbox(Optim.LBFGS(linesearch = LineSearches.BackTracking(order = 3))),
    # Optim.Options(show_trace = true,
    # store_trace = true,
    ...
    @test isapprox(get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[[6,1]], standard_deviation = RBC_CME.constants.timings.var[[2,5]], algorithm = :pruned_third_order)[:standard_deviation], [.0008,.21], atol=1e-6)

    # function combining targets for SS and St.Dev.
    function get_variances_optim(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:2], non_stochastic_steady_state = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]])
        sum(abs2,[out[:non_stochastic_steady_state][1] - 1.45, out[:standard_deviation][1] - .2])
    end
    out = get_variances_optim([.157,.999])

    out = get_statistics(RBC_CME, [.157,.999], parameters = RBC_CME.parameters[1:2], non_stochastic_steady_state = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]])
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:2], non_stochastic_steady_state = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]])
    @test isapprox([out[:non_stochastic_steady_state],out[:standard_deviation]],[[1.45],[.2]],atol = 1e-6)

    # pruned second order
    function get_variances_optim_2nd(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:2], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_second_order)
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:2], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_second_order)

    @test isapprox([out[:mean],out[:standard_deviation]],[[1.45],[.2]],atol = 1e-6)

    # pruned third order
    function get_variances_optim_3rd(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:2], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_third_order)
        sum(abs2,[out[:mean][1] - 1.45, out[:standard_deviation][1] - .2])
    end
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:2], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_third_order)
    ...
    function get_variances_optim2(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:3], non_stochastic_steady_state = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]])
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:3], non_stochastic_steady_state = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]])
    ...
    function get_variances_optim2_2nd(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:3], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_second_order)
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:3], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_second_order)

    @test isapprox([out[:mean], out[:standard_deviation], sol.minimizer[3]],[[1.45],[.2],.02], atol = 1e-6)

    # pruned third order
    function get_variances_optim2_3rd(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:3], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_third_order)
        sum(abs2,[out[:mean][1] - 1.45, out[:standard_deviation][1] - .2, x[3] - .02])
    end
    out = get_variances_optim2([.157,.999,.022])
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:3], mean = [RBC_CME.constants.timings.var[6]], standard_deviation = [RBC_CME.constants.timings.var[5]], algorithm = :pruned_third_order)

    @test isapprox([out[:mean], out[:standard_deviation], sol.minimizer[3]],[[1.45],[.2],.02], atol = 1e-6)

    get_statistics(RBC_CME, RBC_CME.parameter_values[1:3], parameters = RBC_CME.parameters[1:3], non_stochastic_steady_state = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1)

    # function combining targets for SS/mean, St.Dev., autocorrelation and parameter
    function get_variances_optim3(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:4], non_stochastic_steady_state = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1)
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:4], non_stochastic_steady_state = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1)
    ...
    function get_variances_optim3_2nd(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:4], mean = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1, algorithm = :pruned_second_order)
    ...
    out = get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:4], mean = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1, algorithm = :pruned_second_order)
    ...
    function get_variances_optim3_3rd(x)
        out = get_statistics(RBC_CME, x, parameters = RBC_CME.parameters[1:4], mean = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1, algorithm = :pruned_third_order)
        sum(abs2,vcat(out[:mean] - [1.2,1.4], out[:standard_deviation] - [.013,.2], out[:autocorrelation][:,1] - [.955,.997], x[3] - .0215))
    end
    ...
    get_statistics(RBC_CME, sol.minimizer, parameters = RBC_CME.parameters[1:4], mean = RBC_CME.constants.timings.var[[4,6]], standard_deviation = RBC_CME.constants.timings.var[4:5], autocorrelation = RBC_CME.constants.timings.var[[3,5]], autocorrelation_periods = 1:1, algorithm = :pruned_third_order)
