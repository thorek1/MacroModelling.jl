...
@model Ascari_Sbordone_2014 begin
	...
	MC_real[0] = w[0] / (1 - alpha) * exp(A[0]) ^ (1 / (alpha - 1)) * y[0] ^ (alpha / (1 - alpha))

	real_interest[0] = (1 + i[0]) / pi[1]

	Utility[0] = log(y[0]) - d_n * exp(zeta[0]) * N[0] ^ (1 + phi_par) / (1 + phi_par) + beta * Utility[1]

	v[0] = rho_v * v[-1] + sigma__v * e_v[x]

	A[0] = rho_a * A[-1] + sigma__a * e_a[x]

	zeta[0] = rho_zeta * zeta[-1] + sigma__zeta * e_zeta[x]
...
@parameters Ascari_Sbordone_2014 begin
	...
	Pi_bar = (1+trend_inflation/100)^0.25
