...
@model NAWM_EAUS_2008 begin
	EA_R[0] ^ 4 - 1 = EA_PHIRR * (EA_R[-1] ^ 4 - 1) + (1 - EA_PHIRR) * (EA_RRSTAR ^ 4 * EA_PI4TARGET - 1 + EA_PHIRPI * (EA_PIC4[0] - EA_PI4TARGET)) + EA_PHIRGY * (EA_Y[0] / EA_Y[-1] - 1) + sigma__EA_R * EA_EPSR[x]

	US_R[0] ^ 4 - 1 = US_PHIRR * (US_R[-1] ^ 4 - 1) + (1 - US_PHIRR) * (US_RRSTAR ^ 4 * US_PI4TARGET - 1 + US_PHIRPI * (US_PIC4[0] - US_PI4TARGET)) + US_PHIRGY * (US_Y[0] / US_Y[-1] - 1) + sigma__US_R * US_EPSR[x]
	...
	EA_GAMMAU[0] = ((EA_BETA ^ (-1) + EA_DELTA - 1) * EA_QBAR - EA_DELTA * EA_TAUKBAR * EA_PIBAR) / (EA_PIBAR * (1 - EA_TAUKBAR)) * (EA_U[0] - 1) + EA_GAMMAU2 / 2 * (EA_U[0] - 1) ^ 2

	EA_GAMMAUDER[0] = ((EA_BETA ^ (-1) + EA_DELTA - 1) * EA_QBAR - EA_DELTA * EA_TAUKBAR * EA_PIBAR) / (EA_PIBAR * (1 - EA_TAUKBAR)) + (EA_U[0] - 1) * EA_GAMMAU2
	...
	EA_Q[0] = EA_BETA * EA_LAMBDAI[1] / EA_LAMBDAI[0] * ((1 - EA_TAUK[1]) * (EA_RK[1] * EA_U[1] - EA_GAMMAU[1] * EA_PI[1]) + EA_TAUK[1] * EA_DELTA * EA_PI[1] + (1 - EA_DELTA) * EA_Q[1])
	...
	EA_GI[0] = EA_LAMBDAI[0] * EA_NDI[0] * (1 - EA_TAUN[0] - EA_TAUWH[0]) * EA_WI[0] ^ EA_ETAI + EA_BETA * EA_XII * (EA_PIC[1] / (EA_PIC[0] ^ EA_CHII * EA_PI4TARGET ^ (0.25 * (1 - EA_CHII)))) ^ (EA_ETAI - 1) * EA_GI[1]
	...
	EA_CJ[0] * (1 + EA_TAUC[0] + EA_GAMMAVJ[0]) + EA_MJ[0] = (1 - EA_TAUN[0] - EA_TAUWH[0]) * EA_NJ[0] * EA_WJ[0] + EA_TRJ[0] - EA_TJ[0] + EA_MJ[-1] * EA_PIC[0] ^ (-1)
	...
	EA_GJ[0] = EA_LAMBDAJ[0] * (1 - EA_TAUN[0] - EA_TAUWH[0]) * EA_NDJ[0] * EA_WJ[0] ^ EA_ETAJ + EA_BETA * EA_XIJ * (EA_PIC[1] / (EA_PIC[0] ^ EA_CHIJ * EA_PI4TARGET ^ (0.25 * (1 - EA_CHIJ)))) ^ (EA_ETAJ - 1) * EA_GJ[1]
	...
	EA_D[0] = EA_Y[0] * EA_PY[0] - EA_RK[0] * EA_KD[0] - EA_W[0] * EA_ND[0] * (1 + EA_TAUWF[0])

	EA_PHTILDE[0] / EA_PH[0] = EA_THETA / (EA_THETA - 1) * EA_FH[0] / EA_GH[0]

	EA_FH[0] = EA_MC[0] * EA_H[0] + EA_BETA * EA_LAMBDAI[1] * EA_XIH / EA_LAMBDAI[0] * (EA_PIH[1] / (EA_PIH[0] ^ EA_CHIH * EA_PI4TARGET ^ (0.25 * (1 - EA_CHIH)))) ^ EA_THETA * EA_FH[1]

	EA_GH[0] = EA_PH[0] * EA_H[0] + EA_BETA * EA_LAMBDAI[1] * EA_XIH / EA_LAMBDAI[0] * (EA_PIH[1] / (EA_PIH[0] ^ EA_CHIH * EA_PI4TARGET ^ (0.25 * (1 - EA_CHIH)))) ^ (EA_THETA - 1) * EA_GH[1]
	...
	EA_FX[0] = EA_MC[0] * US_SIZE / EA_SIZE * US_IM[0] + EA_BETA * EA_LAMBDAI[1] * EA_XIX / EA_LAMBDAI[0] * (US_PIIM[1] / (US_PIIM[0] ^ EA_CHIX * EA_PI4TARGET ^ (0.25 * (1 - EA_CHIX)))) ^ EA_THETA * EA_FX[1]

	EA_GX[0] = US_PIM[0] * US_SIZE * US_IM[0] * EAUS_RER[0] / EA_SIZE + EA_BETA * EA_LAMBDAI[1] * EA_XIX / EA_LAMBDAI[0] * (US_PIIM[1] / (US_PIIM[0] ^ EA_CHIX * EA_PI4TARGET ^ (0.25 * (1 - EA_CHIX)))) ^ (EA_THETA - 1) * EA_GX[1]
	...
	US_PIIM[0] = US_PIM[0] * US_PIC[0] / US_PIM[-1]
	...
	EA_GAMMAIMCDAG[0] = 1 - EA_GAMMAIMC[0] - (EA_IMC[0] / EA_QC[0] / (EA_IMC[-1] / EA_QC[-1]) - 1) * EA_IMC[0] * EA_GAMMAIMC1 / EA_QC[0] / (EA_IMC[-1] / EA_QC[-1])
	...
	EA_PH[-1] * EA_G[-1] + EA_TR[-1] + EA_B[-1] * EA_PIC[-1] ^ (-1) + EA_PIC[-1] ^ (-1) * AUX_ENDO_LAG_51_1[-1] = EA_TAUC[-1] * EA_C[-1] + (EA_TAUN[-1] + EA_TAUWH[-1]) * (EA_WI[-1] * EA_NDI[-1] + EA_WJ[-1] * EA_NDJ[-1]) + EA_TAUWF[-1] * EA_W[-1] * EA_ND[-1] + EA_TAUK[-1] * (EA_RK[-1] * EA_U[-1] - (EA_DELTA + EA_GAMMAU[-1]) * EA_PI[-1]) * EA_K[-1] + EA_TAUD[-1] * EA_D[-1] + EA_T[-1] + EA_R[-1] ^ (-1) * EA_B[0] + EA_M[-1]

	EA_PH[0] * EA_G[0] = EA_GY[0] * EA_PYBAR * EA_YBAR

	EA_TR[0] = EA_PYBAR * EA_YBAR * EA_TRY[0]
	...
	EA_PIC4[0] = EA_PIC[0] * EA_PIC[-1] * AUX_ENDO_LAG_63_1[-1] * AUX_ENDO_LAG_63_2[-1]
	...
	EA_TRJ[0] = EA_TR[0] / EA_OMEGA - (1 - EA_OMEGA) * EA_TRI[0] / EA_OMEGA

	EA_TJ[0] = EA_T[0] / EA_OMEGA - (1 - EA_OMEGA) * EA_TI[0] / EA_OMEGA

	EA_GAMMAV[0] = (1 - EA_OMEGA) * EA_CI[0] * EA_GAMMAVI[0] + EA_OMEGA * EA_CJ[0] * EA_GAMMAVJ[0]
	...
	EA_YS[0] = EA_H[0] * EA_SH[0] + US_SIZE * US_IM[0] * EA_SX[0] / EA_SIZE

	EA_H[0] = EA_HI[0] + EA_HC[0] + EA_G[0]
	...
	EA_Y[0] * EA_PY[0] = EA_QC[0] + US_PIM[0] * US_SIZE * US_IM[0] * EAUS_RER[0] / EA_SIZE + EA_PH[0] * EA_G[0] + EA_PI[0] * EA_QI[0] - EA_PIM[0] * ((1 - EA_GAMMAIMC[0]) * EA_IMC[0] / EA_GAMMAIMCDAG[0] + (1 - EA_GAMMAIMI[0]) * EA_IMI[0] / EA_GAMMAIMIDAG[0])

	EA_Y[0] = EA_YS[0]

	log(EA_Z[0]) = (1 - EA_RHOZ) * log(EA_ZBAR) + EA_RHOZ * log(EA_Z[-1]) + sigma__EA_Z * EA_EPSZ[x]

	EA_GY[0] = (1 - EA_RHOG) * EA_GYBAR + EA_RHOG * EA_GY[-1] + sigma__EA_G * EA_EPSG[x]

	EA_TRY[0] = (1 - EA_RHOTR) * EA_TRYBAR + EA_RHOTR * EA_TRY[-1] + sigma__EA_TR * EA_EPSTR[x]

	EA_TAUC[0] = (1 - EA_RHOTAUC) * EA_TAUCBAR + EA_TAUC[-1] * EA_RHOTAUC + sigma__EA_TAUC * EA_EPSTAUC[x]

	EA_TAUD[0] = (1 - EA_RHOTAUD) * EA_TAUDBAR + EA_TAUD[-1] * EA_RHOTAUD + sigma__EA_TAUD * EA_EPSTAUD[x]

	EA_TAUK[0] = EA_TAUKBAR * (1 - EA_RHOTAUK) + EA_TAUK[-1] * EA_RHOTAUK + sigma__EA_TAUK * EA_EPSTAUK[x]

	EA_TAUN[0] = (1 - EA_RHOTAUN) * EA_TAUNBAR + EA_TAUN[-1] * EA_RHOTAUN + sigma__EA_TAUN * EA_EPSTAUN[x]

	EA_TAUWH[0] = (1 - EA_RHOTAUWH) * EA_TAUWHBAR + EA_TAUWH[-1] * EA_RHOTAUWH + sigma__EA_TAUWH * EA_EPSTAUWH[x]

	EA_TAUWF[0] = (1 - EA_RHOTAUWF) * EA_TAUWFBAR + EA_TAUWF[-1] * EA_RHOTAUWF + sigma__EA_TAUWF * EA_EPSTAUWF[x]
	...
	US_GAMMAU[0] = ((US_BETA ^ (-1) + US_DELTA - 1) * US_QBAR - US_DELTA * US_TAUKBAR * US_PIBAR) / (US_PIBAR * (1 - US_TAUKBAR)) * (US_U[0] - 1) + US_GAMMAU2 / 2 * (US_U[0] - 1) ^ 2

	US_GAMMAUDER[0] = ((US_BETA ^ (-1) + US_DELTA - 1) * US_QBAR - US_DELTA * US_TAUKBAR * US_PIBAR) / (US_PIBAR * (1 - US_TAUKBAR)) + (US_U[0] - 1) * US_GAMMAU2
	...
	US_Q[0] = US_BETA * US_LAMBDAI[1] / US_LAMBDAI[0] * ((1 - US_TAUK[1]) * (US_RK[1] * US_U[1] - US_GAMMAU[1] * US_PI[1]) + US_TAUK[1] * US_DELTA * US_PI[1] + (1 - US_DELTA) * US_Q[1])
	...
	US_GI[0] = US_LAMBDAI[0] * US_NDI[0] * (1 - US_TAUN[0] - US_TAUWH[0]) * US_WI[0] ^ US_ETAI + US_BETA * US_XII * (US_PIC[1] / (US_PIC[0] ^ US_CHII * US_PI4TARGET ^ (0.25 * (1 - US_CHII)))) ^ (US_ETAI - 1) * US_GI[1]
	...
	US_CJ[0] * (1 + US_TAUC[0] + US_GAMMAVJ[0]) + US_MJ[0] = (1 - US_TAUN[0] - US_TAUWH[0]) * US_NJ[0] * US_WJ[0] + US_TRJ[0] - US_TJ[0] + US_MJ[-1] * US_PIC[0] ^ (-1)
	...
	US_GJ[0] = US_LAMBDAJ[0] * (1 - US_TAUN[0] - US_TAUWH[0]) * US_NDJ[0] * US_WJ[0] ^ US_ETAJ + US_BETA * US_XIJ * (US_PIC[1] / (US_PIC[0] ^ US_CHIJ * US_PI4TARGET ^ (0.25 * (1 - US_CHIJ)))) ^ (US_ETAJ - 1) * US_GJ[1]
	...
	US_D[0] = US_Y[0] * US_PY[0] - US_RK[0] * US_KD[0] - US_W[0] * US_ND[0] * (1 + US_TAUWF[0])

	US_PHTILDE[0] / US_PH[0] = US_THETA / (US_THETA - 1) * US_FH[0] / US_GH[0]

	US_FH[0] = US_MC[0] * US_H[0] + US_BETA * US_LAMBDAI[1] * US_XIH / US_LAMBDAI[0] * (US_PIH[1] / (US_PIH[0] ^ US_CHIH * US_PI4TARGET ^ (0.25 * (1 - US_CHIH)))) ^ US_THETA * US_FH[1]

	US_GH[0] = US_PH[0] * US_H[0] + US_BETA * US_LAMBDAI[1] * US_XIH / US_LAMBDAI[0] * (US_PIH[1] / (US_PIH[0] ^ US_CHIH * US_PI4TARGET ^ (0.25 * (1 - US_CHIH)))) ^ (US_THETA - 1) * US_GH[1]
	...
	US_FX[0] = EA_SIZE * EA_IM[0] * US_MC[0] / US_SIZE + US_BETA * US_LAMBDAI[1] * US_XIX / US_LAMBDAI[0] * (EA_PIIM[1] / (EA_PIIM[0] ^ US_CHIX * US_PI4TARGET ^ (0.25 * (1 - US_CHIX)))) ^ US_THETA * US_FX[1]

	US_GX[0] = EA_PIM[0] * EA_SIZE * EA_IM[0] * USEA_RER[0] / US_SIZE + US_BETA * US_LAMBDAI[1] * US_XIX / US_LAMBDAI[0] * (EA_PIIM[1] / (EA_PIIM[0] ^ US_CHIX * US_PI4TARGET ^ (0.25 * (1 - US_CHIX)))) ^ (US_THETA - 1) * US_GX[1]
	...
	US_GAMMAIMCDAG[0] = 1 - US_GAMMAIMC[0] - (US_IMC[0] / US_QC[0] / (US_IMC[-1] / US_QC[-1]) - 1) * US_IMC[0] * US_GAMMAIMC1 / US_QC[0] / (US_IMC[-1] / US_QC[-1])
	...
	US_PH[-1] * US_G[-1] + US_TR[-1] + US_B[-1] * US_PIC[-1] ^ (-1) + US_PIC[-1] ^ (-1) * AUX_ENDO_LAG_165_1[-1] = US_TAUC[-1] * US_C[-1] + (US_TAUN[-1] + US_TAUWH[-1]) * (US_WI[-1] * US_NDI[-1] + US_WJ[-1] * US_NDJ[-1]) + US_TAUWF[-1] * US_W[-1] * US_ND[-1] + US_TAUK[-1] * (US_RK[-1] * US_U[-1] - (US_DELTA + US_GAMMAU[-1]) * US_PI[-1]) * US_K[-1] + US_TAUD[-1] * US_D[-1] + US_T[-1] + US_R[-1] ^ (-1) * US_B[0] + US_M[-1]

	US_PH[0] * US_G[0] = US_GY[0] * US_PYBAR * US_YBAR

	US_TR[0] = US_PYBAR * US_YBAR * US_TRY[0]
	...
	US_PIC4[0] = US_PIC[0] * US_PIC[-1] * AUX_ENDO_LAG_177_1[-1] * AUX_ENDO_LAG_177_2[-1]
	...
	US_TRJ[0] = US_TR[0] / US_OMEGA - (1 - US_OMEGA) * US_TRI[0] / US_OMEGA

	US_TJ[0] = US_T[0] / US_OMEGA - (1 - US_OMEGA) * US_TI[0] / US_OMEGA

	US_GAMMAV[0] = (1 - US_OMEGA) * US_CI[0] * US_GAMMAVI[0] + US_OMEGA * US_CJ[0] * US_GAMMAVJ[0]
	...
	US_YS[0] = US_H[0] * US_SH[0] + EA_SIZE * EA_IM[0] * US_SX[0] / US_SIZE

	US_H[0] = US_HI[0] + US_HC[0] + US_G[0]
	...
	US_Y[0] * US_PY[0] = US_QC[0] + EA_PIM[0] * EA_SIZE * EA_IM[0] * USEA_RER[0] / US_SIZE + US_PH[0] * US_G[0] + US_PI[0] * US_QI[0] - US_PIM[0] * ((1 - US_GAMMAIMC[0]) * US_IMC[0] / US_GAMMAIMCDAG[0] + (1 - US_GAMMAIMI[0]) * US_IMI[0] / US_GAMMAIMIDAG[0])

	US_Y[0] = US_YS[0]

	log(US_Z[0]) = (1 - US_RHOZ) * log(US_ZBAR) + US_RHOZ * log(US_Z[-1]) + sigma__US_Z * US_EPSZ[x]

	US_GY[0] = (1 - US_RHOG) * US_GYBAR + US_RHOG * US_GY[-1] + sigma__US_G * US_EPSG[x]

	US_TRY[0] = (1 - US_RHOTR) * US_TRYBAR + US_RHOTR * US_TRY[-1] + sigma__US_TR * US_EPSTR[x]

	US_TAUC[0] = (1 - US_RHOTAUC) * US_TAUCBAR + US_TAUC[-1] * US_RHOTAUC + sigma__US_TAUC * US_EPSTAUC[x]

	US_TAUD[0] = (1 - US_RHOTAUD) * US_TAUDBAR + US_TAUD[-1] * US_RHOTAUD + sigma__US_TAUD * US_EPSTAUD[x]

	US_TAUK[0] = US_TAUKBAR * (1 - US_RHOTAUK) + US_TAUK[-1] * US_RHOTAUK + sigma__US_TAUK * US_EPSTAUK[x]

	US_TAUN[0] = (1 - US_RHOTAUN) * US_TAUNBAR + US_TAUN[-1] * US_RHOTAUN + sigma__US_TAUN * US_EPSTAUN[x]

	US_TAUWH[0] = (1 - US_RHOTAUWH) * US_TAUWHBAR + US_TAUWH[-1] * US_RHOTAUWH + sigma__US_TAUWH * US_EPSTAUWH[x]

	US_TAUWF[0] = (1 - US_RHOTAUWF) * US_TAUWFBAR + US_TAUWF[-1] * US_RHOTAUWF + sigma__US_TAUWF * US_EPSTAUWF[x]
	...
	1 = US_R[0] * EA_BETA * EA_LAMBDAI[1] * (1 - EA_GAMMAB[0]) / EA_LAMBDAI[0] * EA_RERDEP[1] / US_PIC[1]

	EA_GAMMAB[0] = EA_GAMMAB1 * (exp(EA_RER[0] * EA_BF[0] / US_PIC[0] / (EA_Y[0] * EA_PY[0]) - EA_BFYTARGET) - 1) - EA_RP[0]

	EA_RP[0] = EA_RHORP * EA_RP[-1] + sigma__EA_RP * EA_EPSRP[x]

	EA_RERDEP[0] = EA_RER[0] / EA_RER[-1]

	EA_TOT[0] = EA_PIM[0] / (US_PIM[0] * EA_RER[0])

	EA_TB[0] = US_PIM[0] * US_SIZE * US_IM[0] * EA_RER[0] / EA_SIZE - EA_PIM[0] * EA_IM[0]

	EA_BF[0] / US_R[-1] = EA_BF[-1] + EA_TB[-1] / EA_RER[-1]

	EA_SIZE * EA_BF[0] + US_SIZE * US_BF[0] = 0

	AUX_ENDO_LAG_51_1[0] = EA_M[-1]

	AUX_ENDO_LAG_63_1[0] = EA_PIC[-1]

	AUX_ENDO_LAG_63_2[0] = AUX_ENDO_LAG_63_1[-1]

	AUX_ENDO_LAG_165_1[0] = US_M[-1]

	AUX_ENDO_LAG_177_1[0] = US_PIC[-1]

	AUX_ENDO_LAG_177_2[0] = AUX_ENDO_LAG_177_1[-1]
