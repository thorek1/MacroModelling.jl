...
Turing.@model function loglikelihood_scaling_function(m, data, observables, Ω)
    ...
    δ     ~ MacroModelling.Normal(0.02, 0.05)
    # γ     ~ Turing.Normal(1, 0.05)
    σ     ~ MacroModelling.Normal(0.01, 0.05)

    # α ~ Turing.Uniform(0.15, 0.45)
    # β ~ Turing.Uniform(0.92, 0.9999)
    # δ ~ Turing.Uniform(0.0, 1.0)
    # σ ~ Turing.Uniform(0.0, 1.0)
    ...
    states ~ Turing.MvNormal(vec(state[observables_index,:] .+ solution[1][observables_index...]), Ω * ℒ.I(length(state[observables_index,:])))
...
using MuseInference, StatsPlots
import Turing
using AbstractDifferentiation

model = loglikelihood_scaling | (states = vec(simulated_data(:k,:,:Shock_matrix)'),)
prob = MuseInference.TuringMuseProblem(model, params=(:δ, :σ))

result = muse(prob, (δ=0.2, σ=0.01,))

# get solution
result = muse(prob, (δ=0.5, σ=0.1))
