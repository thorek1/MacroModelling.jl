...
@model Gali_Monacelli_2005_CITR begin
	...
	ynat[0] = Gamma * a[0] + Psi * alpha * ystar[0]
	...
	nx[0] = alpha * s[0] * (omega / sigma - 1)

	y[0] = c[0] + alpha * s[0] * omega / sigma

	real_wage[0] = sigma * c[0] + n[0] * phi

	a[0] = rho__a * a[-1] + epsilon__a[x]

	ystar[0] = rho__y * ystar[-1] + epsilon__star[x]
...
@parameters Gali_Monacelli_2005_CITR begin
	...
	Theta = sigma*gamma+(1-alpha)*(sigma*eta-1)-1
