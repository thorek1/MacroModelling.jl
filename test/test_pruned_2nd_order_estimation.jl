...
Turing.@model function FS2000_loglikelihood_function(data, m, algorithm, on_failure_likelihood)
...
samps = @time sample(FS2000_loglikelihood_function(data, FS2000, :pruned_second_order, -Inf), NUTS(adtype = ADTypes.AutoZygote()), n_samples, progress = true, initial_params = FS2000.parameter_values)
