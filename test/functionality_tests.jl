function functionality_test(m; algorithm = :first_order, plots = true)
        ...
        if algorithm == :first_order
            for parameter_values in [old_params, old_params .* exp.(rand(length(old_params))*1e-4)]
                ...
                for initial_state in init_states
                    ...
                    deriv_fin = FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6), x->get_irf(m, x, initial_state = initial_state)[:,1,1], parameter_values)
    ...
    @testset "get_solution with parameter input" begin
        for parameter_values in [old_params, old_params .* exp.(rand(length(old_params))*1e-4)]
            ...
            deriv_sol_fin = []
            for i in 1:length(sol)-2
                push!(deriv_sol_fin, FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6),
    ...
    @testset "get_statistics" begin
        ...
        deriv1_fin = FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6),
        ...
        if algorithm ∈ [:first_order, :pruned_second_order, :pruned_third_order]
            ...
            deriv2_fin = FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6),
                                                            x->get_statistics(m, x, algorithm = algorithm, 
                                                            mean = m.var)[:mean], old_params)
            ...
            deriv3_fin = FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6),
                                                            x->get_statistics(m, x, algorithm = algorithm, 
                                                            standard_deviation = m.var)[:standard_deviation], old_params)
            ...
            deriv4_fin = FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6),
                                                            x->get_statistics(m, x, algorithm = algorithm, 
                                                            variance = m.var)[:variance], old_params)
            ...
            deriv5_fin = FiniteDifferences.jacobian(FiniteDifferences.forward_fdm(3,1, max_range = 1e-6),
                                                            x->get_statistics(m, x, algorithm = algorithm, 
                                                            tol = MacroModelling.Tolerances(NSSS_xtol = 1e-14, lyapunov_acceptance_tol = 1e-14, 
                                                            sylvester_acceptance_tol = 1e-14),
                                                            covariance = m.var)[:covariance], old_params)
