...
function build_Γ₂(m, C2z0::AbstractMatrix, Ey::Vector)

    nu = m.timings.nExo

    nx = m.timings.nPast_not_future_and_mixed
        
    nx2 = nx*(nx+1)/2 |> Int

    nu2 = nu*(nu+1)/2 |> Int
    
    u_u = reshape([[(i, i == k ? 1 : 0), (k, i == k ? 1 : 0)] for k in m.timings.exo for i in m.timings.exo],nu,nu)
    ...
    if size(C2z0) == (nu,nu) 
        ...
        inputs = vcat(m.timings.exo, upper_triangle(u_u), vec(xf_u))
    ...
    for (i¹,s¹) in enumerate(inputs)
        for (i²,s²) in enumerate(inputs)
            terms1 = [c isa Symbol ? c : c[1] ∈ m.timings.exo ? c[1] : c for c in (s¹ isa Symbol ? [s¹] : s¹)]
            terms2 = [c isa Symbol ? c : c[1] ∈ m.timings.exo ? c[1] : c for c in (s² isa Symbol ? [s²] : s²)]

            intrsct = intersecting_elements(terms1,terms2)

            intrsct_cnts = Dict([element => count(==(element),intrsct) for element in unique(intrsct)])
            
            ϵ²_terms = filter(u -> all([(v isa Symbol ? 0 : v[1]) ∈ m.timings.exo for v in u]) && u[1][2] == 1,[(s¹ isa Symbol ? [s¹] : s¹), (s² isa Symbol ? [s²] : s²)])

            terms = filter(u -> !(all([(v isa Symbol ? 0 : v[1]) ∈ m.timings.exo for v in u]) && u[1][2] == 1),[(s¹ isa Symbol ? [s¹] : s¹), (s² isa Symbol ? [s²] : s²)])

            if length(ϵ²_terms) > 0
                ϵ²_combo = [c isa Symbol ? c : c[1] ∈ m.timings.exo ? c[1] : c for c in reduce(vcat,ϵ²_terms)]
            ...
            if length(terms) > 0
                combo = [c isa Symbol ? c : c[1] ∈ m.timings.exo ? c[1] : c for c in reduce(vcat,terms)]
            ...
            if any([k ∈ m.timings.exo && v == 1 for (k,v) in combined_combo_cnts])
                continue
            ...
            elseif all([k ∈ m.timings.exo && v == 4 for (k,v) in combined_combo_cnts]) && 
                all([k ∈ m.timings.exo && v == 2 for (k,v) in intrsct_cnts])

                Γ[i¹,i²] = 2

            elseif length(setdiff(keys(combined_combo_cnts),m.timings.exo)) == 0 && 
                length(intrsct_cnts) > 0 && 
            ...
            elseif all([k ∈ m.timings.exo && v == 6 for (k,v) in combined_combo_cnts]) && 
                all([k ∈ m.timings.exo && v == 3 for (k,v) in intrsct_cnts])

                Γ[i¹,i²] = 15
                
            elseif length(filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)) > 0 && 
                sum(values(filter(((j,u),) -> !(j ∈ m.timings.exo), combined_combo_cnts))) == 2 && 
                all([i[2] == 1 for i in keys(filter(((j,u),) -> !(j ∈ m.timings.exo), combined_combo_cnts))]) && 
                all([k ∈ m.timings.exo && v >= 1 for (k,v) in  filter(((j,u),) -> j ∈ m.timings.exo, intrsct_cnts)])

                if all([v == 2 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)])
                ...
                elseif all([v == 4 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)])
                    indices = [indexin([i[1]], m.timings.past_not_future_and_mixed)[1] for i in setdiff(keys(combined_combo_cnts), m.timings.exo)]
            ...
            elseif length(filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)) > 0 && # at least one shock 
                sum(values(filter(((j,u),) -> !(j ∈ m.timings.exo), combined_combo_cnts))) == 2 && # non shocks have max two entries
                all([i[2] == 2 for i in keys(filter(((j,u),) -> !(j ∈ m.timings.exo), combined_combo_cnts))]) && # non shocks are all double entries
                all([k ∈ m.timings.exo && v >= 1 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, intrsct_cnts)]) # all shocks appear in both entries

                vars = setdiff(keys(combined_combo_cnts), m.timings.exo)
            ...
            elseif length(filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)) > 0 && # at least one shock 
                length(filter(((j,u),) -> !(j ∈ m.timings.exo) && j[2] == 2, combined_combo_cnts)) == 1 &&
                sum(values(filter(((j,u),) -> !(j ∈ m.timings.exo), combined_combo_cnts))) > 2 && # non shocks have more than two entries
                all([k ∈ m.timings.exo && v >= 1 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, intrsct_cnts)]) # all shocks appear in both entries

                indices_second_mean = [indexin([i[1][1]], intersect(model_order,m.var))[1] for i in filter(((j,u),) -> !(j ∈ m.timings.exo) && u == 1 && j[2] == 2, combined_combo_cnts)][1]

                indices_first_variance = [indexin([i[1][1]], m.timings.past_not_future_and_mixed)[1] for i in filter(((j,u),) -> !(j ∈ m.timings.exo) && j[2] == 1, combined_combo_cnts)]
                ...
                indices_second = [indexin([i[1][1]], m.timings.past_not_future_and_mixed)[1] for i in filter(((j,u),) -> !(j ∈ m.timings.exo) && u == 1 && j[2] == 2, combined_combo_cnts)][1] + m.timings.nPast_not_future_and_mixed

                Γ[i¹,i²] = C2z0[indices_second, indices_first] + C2z0[indices_first_variance[1], indices_first_variance[2]] * Ey[indices_second_mean]

            elseif length(filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)) > 0 && # at least one shock 
                sum(values(filter(((j,u),) -> !(j ∈ m.timings.exo), combined_combo_cnts))) == 4 && # non shocks have four entries
                all([k ∈ m.timings.exo && v >= 1 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, intrsct_cnts)]) # all shocks appear in both entries
            ...
            elseif length(filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)) > 0 && 
                sum(values(filter(((j,u),) -> !(j ∈ m.timings.exo) && j[2] == 2, combined_combo_cnts))) == 1 &&  
                sum(values(filter(((j,u),) -> !(j ∈ m.timings.exo) && j[2] == 1, combined_combo_cnts))) == 0 &&  
                all([k ∈ m.timings.exo && v >= 1 for (k,v) in  filter(((j,u),) -> j ∈ m.timings.exo, intrsct_cnts)])

                indices = [indexin([i[1]], intersect(model_order,m.var))[1] for i in setdiff(keys(combined_combo_cnts), m.timings.exo)][1]
                
                if all([v == 4 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)])
                    Γ[i¹,i²] = 3 * Ey[indices]
                elseif all([v == 2 for (k,v) in filter(((j,u),) -> j ∈ m.timings.exo, combined_combo_cnts)])
                    Γ[i¹,i²] = Ey[indices]
                end
            end
        end
...
build_Γ₂(m, C2z0::AbstractMatrix) = build_Γ₂(m, C2z0::AbstractMatrix, [0])
    ...
    for (i¹,s¹) in enumerate(inputs)
        for (i²,s²) in enumerate(inputs)
            for (i³,s³) in enumerate(inputs)
                ...
                terms = filter(u -> !(all([(v isa Symbol ? 0 : v[1]) ∈ m.timings.exo for v in u]) && u[1][2] == 1),[(s¹ isa Symbol ? [s¹] : s¹), (s² isa Symbol ? [s²] : s²), (s³ isa Symbol ? [s³] : s³)])
