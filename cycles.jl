...
md"# The simplest case: only backward looking"
...
get_solution(reduced_form) |> show_pluto

# ╔═╡ 4199411e-38ef-4f8e-95ff-8d10cb0a909f
md"The autoregressive coefficient on investment is larger than 1 for the first order solution."

# ╔═╡ 92c019c2-8540-431c-9972-93a6be2fdd38
get_solution(reduced_form, algorithm = :second_order) |> show_pluto

# ╔═╡ 42a61485-e3ba-4a37-8c50-18381c2dbd95
md"The second order terms are negative due to the non-zero NSSS."

# ╔═╡ c2f25861-5fff-47d1-bf4b-5671d61c0f45
get_solution(reduced_form, algorithm = :third_order) |> show_pluto

# ╔═╡ 1508faa7-3e2e-4f3a-90e1-6616b1544610
md"## Plotting"

# ╔═╡ ac41c574-c53e-4127-918b-5be4040cc601
md"The paths for the first order solution are explosive. We should see this in the eigenvalues of the Schur (QZ) decomposition:"

# ╔═╡ 383372a0-dc81-4343-bff8-74fe688e96e1
@bind a Slider(0:10:1000, default = 100)

# ╔═╡ 180918d3-ebc8-4200-8a21-a9b926797368
a

# ╔═╡ d51f5759-ee26-4d1a-a455-6f2f523e7f1b
...
# ╔═╡ ad88b4bf-c614-4e0e-a6b2-dc485a7fe3b1
get_eigenvalues(reduced_form)

# ╔═╡ 5c17241c-0a5e-44ab-a71b-b77d0ad63e11
md"There is one real eigenvalue > 1 and one within the unit circle. Note that this model has no forward looking (jump) variables. Therefore it does not satisfy the Blanchard-Kahn conditions. According to Galizia (2022) it might exhibit limit cycles using third or higher order perturbation."

# ╔═╡ 7259ca28-58b7-4447-8e33-1a1d1bcecafd
md"For second order the paths are still explosive."

# ╔═╡ 47901235-dc5d-45a2-908a-b1c00648727c
@bind b Slider(-2.25:.01:1, default = -2)

# ╔═╡ 1a4c7a8a-2f33-4958-a719-439848fbfdfe
plot_irf(reduced_form, initial_state = fill(1e-4, 3), periods = 100, parameters = [:a => a, :b => b])[1]

# ╔═╡ d3db2e0a-bea3-4314-82a7-9267aabed3de
plot_irf(reduced_form, initial_state = fill(1e-4, 3), periods = 100, algorithm = :second_order, parameters = [:a => a, :b => b])[1]

# ╔═╡ f13e7256-b624-41ee-bf4d-aa912875c949
b

# ╔═╡ c0f45cf7-0a8e-42e8-8050-41861da51baf
plot_irf(reduced_form, initial_state = fill(1e-4, 3), periods = 100, algorithm = :third_order, parameters = [:a => a, :b => b])[1]

# ╔═╡ 7e802bf1-e543-4f57-83ab-ad7f0e6655dc
md"Indeed at third order we get (non-trivial) limit cycles. Does this hold as well if we have shocks?
# Stochastic limit cycles"

# ╔═╡ f9bc2196-7075-4334-981d-cb0d197a3292
...
# ╔═╡ 5718c81a-0fa2-49d6-94c3-8e8227daad12
...
# ╔═╡ 45fdbb11-893a-4051-b8b0-257c45c16cdc
SS(reduced_form_stochastic) |> show_pluto

# ╔═╡ 8dd7cfbd-cb54-4de3-a5b6-2b39960e55ef
plot_irf(reduced_form_stochastic,  periods = 100)[1]

# ╔═╡ 1477de09-4e50-4339-a624-7c4e84b42b53
plot_irf(reduced_form_stochastic,  periods = 1000, algorithm = :second_order)[1]

# ╔═╡ 25ba4aab-5195-4e74-be10-b498194498d1
plot_irf(reduced_form_stochastic, periods = 100, algorithm = :third_order)[1]

# ╔═╡ 290651f9-2315-4a3d-aa92-a115bea6b25a
plot_irf(reduced_form_stochastic, periods = 1000, algorithm = :third_order)[1]

# ╔═╡ 78402155-bcbd-47b3-9dc3-47a8a4f494ff
md"With the shock size being small we get the same dynamics."

# ╔═╡ 9422fe5b-8912-418f-941f-3fb8c51e62c0
plot_irf(reduced_form_stochastic, algorithm = :third_order, periods = 100, negative_shock = true)[1]

# ╔═╡ 1ff50a53-9a6e-4dc6-9979-3d8b859e3152
md"Yet there is definitely a basin of attraction and if you go to far away you might not enter the limit cycle as is the case with a negative shock."

# ╔═╡ 74b8daa2-b731-4217-ba9f-84b3f95e0693
md"Does this also work with pruning?"

# ╔═╡ 6df7f697-a6dc-45ac-aa6a-b453572bda8d
plot_irf(reduced_form_stochastic, algorithm = :pruned_third_order, periods = 100)[1]

# ╔═╡ 1c84bf1d-01ee-4653-841e-df8a7ad67615
md"No because there is no dynamic feedback counteracting the explosive first order dynamics."

# ╔═╡ c867d3bc-cef7-478c-815b-d8877c3ab49a
md"# Forward looking stochastic limit cycles.
So far we focused on backward looking models but the interesting part is forward looking economic models. That is where the matching of unstable eigenvalues with forward looking variables should matter as well."

# ╔═╡ afbf827a-01ae-4c46-86d9-5f75abf97bde
...
# ╔═╡ 2e16b9fd-67ca-4b0e-912f-e0e296b5b0b0
...
# ╔═╡ 38a6a4c3-2c22-4326-9fd7-0f9bdb5962f0
md"Here we shifted the timing of investment in the first and last equation. Note that investment appears in expectations now."

# ╔═╡ 0b941d86-bc37-49b2-95d5-7cad0b8d7902
SS(reduced_form_forward_looking_stochastic) |> show_pluto

# ╔═╡ e14b5d73-69d5-47dc-a540-5daef9e00211
get_eigenvalues(reduced_form_forward_looking_stochastic)

# ╔═╡ 7db069fb-d14a-4293-a761-f876f5eb33c2
md"Note that the eigenvalues of the QZ are very different in the forward looking model. We choose another parameterisation in order to have at least one unstable eigenvalue:"

# ╔═╡ da83e580-15c2-43dd-962e-6269cbc36796
@model reduced_form_forward_looking_stoch begin
    K[0] = (1 - δ) * K[-1] + I[0]
    I[0] = α * K[-1] + G[0]
    G[0] = a * I[1] ^ 3 + b * I[1] + σ * ϵ[x]
end

# ╔═╡ b77e6ae0-ace5-4a3c-b4a2-8d2f3bff7a04
@parameters reduced_form_forward_looking_stoch begin
    α = .15
    δ = .02
    σ = .0001
    a = 10
...
# ╔═╡ 12ce5865-1e12-45be-84bb-7fe016f17397
SS(reduced_form_forward_looking_stoch) |> show_pluto

# ╔═╡ 73362a28-37ad-43fb-a257-348fa29fba77
md"Now we do the Taylor approximation around the zero NSSS."

# ╔═╡ 0df34990-2cbb-421d-b28b-9de3c6b7289c
get_eigenvalues(reduced_form_forward_looking_stoch)

# ╔═╡ 9fceddc8-c0ab-475c-a6db-f75ab935788e
md"And we get two unstable eigenvalues in this case. Even though we only have one forward looking variable. As before we have one unstable eigenvalue too much."

# ╔═╡ 67002428-4110-4d2c-9c7a-a9d7380ab810
get_solution(reduced_form_forward_looking_stoch) |> show_pluto

# ╔═╡ 0ea8167b-0bec-4a1a-8d57-ebea2f84d154
plot_irf(reduced_form_forward_looking_stoch)[1]

# ╔═╡ 0dac68c2-878f-4cc4-a1dc-567c1cbb097c
@bind aa Slider(0:10:1000, default = 10)

# ╔═╡ 9ab2da58-7e9c-4bcd-91ed-248fb13a3f04
@bind bb Slider(0:.01:2, default = .5)

# ╔═╡ 2d1ba40e-29ca-4383-898a-24c58d0b9507
plot_irf(reduced_form_forward_looking_stoch, algorithm = :third_order, parameters = [:a => aa, :b => bb], periods = 100)[1]

# ╔═╡ ca75e3c5-f47e-409e-ad82-d17a89d15957
bb

# ╔═╡ 2b7f586f-c502-4aa0-891f-db44bba58f37
md"This parametrisation exhibits limit cycles at third order"

# ╔═╡ dd09252b-b25e-4bbe-993e-16f022bd1b7a
plot_irf(reduced_form_forward_looking_stoch, algorithm = :third_order, negative_shock = true, parameters = [:a => aa, :b => bb], periods = 100)[1]

# ╔═╡ db7990ab-3c16-4ab4-8120-9cb8f5da0e63
md"and it seems to be symmetric around the NSSS."

# ╔═╡ 729dfccc-1d4e-467e-8674-20bef581dd8e
md"# Larger forward looking stochastic limit cycle model
Let's add another state variabel for the shock process."

# ╔═╡ dcfde53e-0579-4cbe-ba57-90c67f73ac93
...
# ╔═╡ bdc4a3bb-2873-4904-8211-82b4ef0af094
...
# ╔═╡ 4cbce23f-32b5-4df3-a183-f296c8e693e1
SS(larger_forward_looking_stochastic) |> show_pluto

# ╔═╡ 7444738c-7afd-4162-8b6b-7a65e1d5daf2
md"We approximate around the zero NSSS."

# ╔═╡ 271b2da3-d268-4d7b-81a3-606af4bf4cbe
get_eigenvalues(larger_forward_looking_stochastic)

# ╔═╡ 6874f079-be63-4f9a-bbc5-67ca2c41671b
md"We have two unstable eigenvalues as before and one stable eigenvalue."

# ╔═╡ ada42dd0-a210-45a9-850b-46c721dd8ae6
plot_irf(larger_forward_looking_stochastic, algorithm = :third_order)[1]

# ╔═╡ 1497fafd-a303-4c0a-a90a-1b78838644f7
md"It appears an extra state does not change the first order solution:"

# ╔═╡ ec2f323a-6df4-4fdb-92e5-78beba0f7354
get_solution(larger_forward_looking_stochastic) |> show_pluto

# ╔═╡ 4596f77c-60bd-48cc-8732-ef6e58d951d9
md"In fact it does change the solution as some of the impact is attributed to the new state variable instead of past values of K.

Note that also in this case he will sort the stable eigenvalue first and the other two unstable eigenvalues are paired.

The case not covered is multiple unique groups of unstable eigenvalues.

First attempts seem difficult. Economics might guide us here."
...
manifest_format = "2.0"
project_hash = "49605b00dd0caa22991bb9b36f0e12186d86cde7"
...
git-tree-sha1 = "23d650690c95537893d7a666854362c0024a6c99"
